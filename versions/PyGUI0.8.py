...
def ensure_same_length(*arrays):
    """Ensure all arrays have the same length by trimming to the shortest one"""
    if not arrays:
        return []
    ...
    return [arr[:min_length] if arr is not None else None for arr in arrays]
...
def parse_ppd_data(header, data_bytes):
    ...
    try:
        ...
        # Ensure all arrays have the same length (in case of inconsistent data)
        analog_1, analog_2, digital_1, digital_2 = ensure_same_length(analog_1, analog_2, digital_1, digital_2)
...
def downsample_data(time, signal, factor=1):
    ...
    """
    if factor <= 1 or signal is None or len(signal) == 0 or time is None or len(time) == 0:
        return time, signal

    try:
        # Ensure time and signal arrays have the same length
        ...
        min_len = min(len(downsampled_time), len(downsampled_signal))
        downsampled_time = downsampled_time[:min_len]
        downsampled_signal = downsampled_signal[:min_len]

        return downsampled_time, downsampled_signal
    except Exception as e:
        print(f"Error in downsample_data: {e}")
        return time, signal  # Return original data if downsampling fails
...
def process_data(time, analog_1, analog_2, digital_1, digital_2,
    ...
    try:
        ...
        if time is None or analog_1 is None or len(time) == 0 or len(analog_1) == 0:
            return None, None, None, None, None, None, None, None

        # Ensure all input arrays have compatible dimensions
        time, analog_1, analog_2, digital_1, digital_2 = ensure_same_length(
        ...
        artifact_mask = None
        if analog_2 is not None and len(analog_2) > 0:
        ...
        # Ensure all result arrays have compatible dimensions
        processed_time, processed_signal, processed_analog_2 = ensure_same_length(
            processed_time, processed_signal, processed_analog_2)

        downsampled_time, downsampled_raw_analog_1, downsampled_raw_analog_2 = ensure_same_length(
            downsampled_time, downsampled_raw_analog_1, downsampled_raw_analog_2)

        if artifact_mask is not None and len(artifact_mask) > len(processed_signal):
            artifact_mask = artifact_mask[:len(processed_signal)]
        ...
        return time, analog_1, analog_2, analog_1, analog_2, digital_1, digital_2, None
...
class PhotometryViewer:
    def __init__(self, root, file_path):
        ...
        try:
            ...
            if (self.processed_time is not None and self.processed_signal is not None and
                    len(self.processed_time) != len(self.processed_signal)):
                print(
                    f"Warning: Dimension mismatch between time ({len(self.processed_time)}) and signal ({len(self.processed_signal)})")
                # Fix by truncating to the shorter length
                min_len = min(len(self.processed_time), len(self.processed_signal))
                self.processed_time = self.processed_time[:min_len]
                self.processed_signal = self.processed_signal[:min_len]
            ...
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Failed to initialize viewer: {str(e)}")
            root.destroy()
    ...
    def create_gui(self):
        try:
            ...
            if self.processed_time is not None and self.processed_signal is not None:
                min_len = min(len(self.processed_time), len(self.processed_signal))
                self.line, = self.ax1.plot(self.processed_time[:min_len] / 60, self.processed_signal[:min_len], 'g-',
                                           lw=1)
            else:
                self.line, = self.ax1.plot([], [], 'g-', lw=1)
            ...
            # Verify dimensions before plotting
            if self.processed_time is not None and self.downsampled_raw_analog_1 is not None:
                min_len = min(len(self.processed_time), len(self.downsampled_raw_analog_1))
                self.raw_line, = self.ax2.plot(
                    self.processed_time[:min_len] / 60,
                    self.downsampled_raw_analog_1[:min_len],
                    'b-', lw=1, label='Analog 1'
                )
            else:
                self.raw_line, = self.ax2.plot([], [], 'b-', lw=1, label='Analog 1')

            if self.downsampled_raw_analog_2 is not None:
                # Verify dimensions before plotting
                if self.processed_time is not None:
                    min_len = min(len(self.processed_time), len(self.downsampled_raw_analog_2))
                    self.raw_line2, = self.ax2.plot(
                        self.processed_time[:min_len] / 60,
                        self.downsampled_raw_analog_2[:min_len],
                        'g-', lw=1, label='Analog 2'
                    )
                else:
                    self.raw_line2, = self.ax2.plot([], [], 'g-', lw=1, label='Analog 2')
            ...
            if self.processed_digital_1 is not None and self.time is not None:
                min_len = min(len(self.time), len(self.processed_digital_1))
                line, = self.ax3.plot(
                    self.time[:min_len] / 60,
                    self.processed_digital_1[:min_len],
                    'b-', lw=1, label='Digital 1'
                )
                self.digital_lines.append(line)

            if self.processed_digital_2 is not None and self.time is not None:
                min_len = min(len(self.time), len(self.processed_digital_2))
                line, = self.ax3.plot(
                    self.time[:min_len] / 60,
                    self.processed_digital_2[:min_len],
                    'r-', lw=1, label='Digital 2'
                )
                self.digital_lines.append(line)
    ...
    def update_artifact_markers(self):
        ...
        try:
            ...
            # Make sure artifact_mask is not longer than processed_signal
            if len(self.artifact_mask) > len(self.processed_signal):
                self.artifact_mask = self.artifact_mask[:len(self.processed_signal)]
            ...
            if hasattr(self, 'artifact_markers_raw') and self.downsampled_raw_analog_2 is not None:
                if len(self.artifact_mask) > len(self.downsampled_raw_analog_2):
                    # Truncate mask if needed
                    mask = self.artifact_mask[:len(self.downsampled_raw_analog_2)]
                else:
                    mask = self.artifact_mask

                artifact_values_raw = self.downsampled_raw_analog_2[mask]
                self.artifact_markers_raw.set_data(artifact_times[:len(artifact_values_raw)], artifact_values_raw)
        except Exception as e:
            print(f"Error updating artifact markers: {e}")
    ...
    def reset_view(self):
        ...
        try:
            if self.time is not None and len(self.time) > 0:
                ...
                raw_data = []
                if self.raw_analog_1 is not None and len(self.raw_analog_1) > 0:
                    raw_data.append(self.raw_analog_1)
                if self.raw_analog_2 is not None and len(self.raw_analog_2) > 0:
    ...
    def update_filter(self, _=None):
        ...
        try:
            ...
            # Verify dimensions
            if self.processed_time is not None and self.processed_signal is not None:
                if len(self.processed_time) != len(self.processed_signal):
                    print(
                        f"Warning: Dimension mismatch: time={len(self.processed_time)}, signal={len(self.processed_signal)}")
                    # Fix dimensions
                    min_len = min(len(self.processed_time), len(self.processed_signal))
                    self.processed_time = self.processed_time[:min_len]
                    self.processed_signal = self.processed_signal[:min_len]
            ...
            if self.processed_time is not None and self.processed_signal is not None:
                if len(self.processed_time) > 0 and len(self.processed_signal) > 0:
                    min_len = min(len(self.processed_time), len(self.processed_signal))
                    self.line.set_xdata(self.processed_time[:min_len] / 60)
                    self.line.set_ydata(self.processed_signal[:min_len])

            # Update raw analog plots with downsampled RAW data
            if self.processed_time is not None and self.downsampled_raw_analog_1 is not None:
                if len(self.processed_time) > 0 and len(self.downsampled_raw_analog_1) > 0:
                    min_len = min(len(self.processed_time), len(self.downsampled_raw_analog_1))
                    self.raw_line.set_xdata(self.processed_time[:min_len] / 60)
                    self.raw_line.set_ydata(self.downsampled_raw_analog_1[:min_len])

            if hasattr(self,
                       'raw_line2') and self.processed_time is not None and self.downsampled_raw_analog_2 is not None:
                if len(self.processed_time) > 0 and len(self.downsampled_raw_analog_2) > 0:
                    min_len = min(len(self.processed_time), len(self.downsampled_raw_analog_2))
                    self.raw_line2.set_xdata(self.processed_time[:min_len] / 60)
                    self.raw_line2.set_ydata(self.downsampled_raw_analog_2[:min_len])

            # Digital signals use original time base
            if len(self.digital_lines) > 0 and self.time is not None and self.processed_digital_1 is not None:
                if len(self.time) > 0 and len(self.processed_digital_1) > 0:
                    min_len = min(len(self.time), len(self.processed_digital_1))
                    self.digital_lines[0].set_xdata(self.time[:min_len] / 60)
                    self.digital_lines[0].set_ydata(self.processed_digital_1[:min_len])

            if len(self.digital_lines) > 1 and self.time is not None and self.processed_digital_2 is not None:
                if len(self.time) > 0 and len(self.processed_digital_2) > 0:
                    min_len = min(len(self.time), len(self.processed_digital_2))
                    self.digital_lines[1].set_xdata(self.time[:min_len] / 60)
                    self.digital_lines[1].set_ydata(self.processed_digital_2[:min_len])
    ...
    def update_y_limits(self):
        ...
        try:
            if self.processed_signal is not None and len(self.processed_signal) > 0:
            ...
            print(f"Error in update_y_limits: {e}")
