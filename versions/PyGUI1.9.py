...
class PhotometryViewer:
    ...
    def create_peak_detection_tab(self):
        """Create the peak detection tab with controls (Focus on Peaks)"""
        # Create the tab
        self.peak_tab = ttk.Frame(self.control_notebook)
        self.control_notebook.add(self.peak_tab, text="Peak Detection")  # Tab name
        ...
        # --- Peak Parameter Sliders with HIGHER DEFAULT VALUES---
        # Prominence slider - default set to higher value (10.0 instead of 2.0)
        tk.Label(self.peak_tab, text="Peak Prominence:", font=('Arial', font_size)).grid(
        ...
        # Width slider - default set to higher value (2.0 instead of 0.0)
        tk.Label(self.peak_tab, text="Min Peak Width (s):", font=('Arial', font_size)).grid(
            row=1, column=0, sticky="w", padx=10, pady=5)
        self.peak_width_slider = tk.Scale(self.peak_tab, from_=0.0, to=10.0,
                                          resolution=0.1, orient=tk.HORIZONTAL,
                                          length=800, width=20, font=('Arial', slider_font_size))
        self.peak_width_slider.set(2.0)  # Higher default value
        self.peak_width_slider.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

        # Distance slider - default set to higher value (5.0 instead of 0.0)
        ...
        self.peak_distance_slider = tk.Scale(self.peak_tab, from_=0.0, to=30.0,
                                             resolution=0.5, orient=tk.HORIZONTAL,
                                             length=800, width=20, font=('Arial', slider_font_size))
        self.peak_distance_slider.set(5.0)  # Higher default value
        self.peak_distance_slider.grid(row=2, column=1, padx=10, pady=5, sticky="ew")

        # Threshold slider - default set to higher value (5.0 instead of 0.0)
        ...
        self.peak_threshold_slider = tk.Scale(self.peak_tab, from_=0.0, to=20.0,
                                              resolution=0.5, orient=tk.HORIZONTAL,
                                              length=800, width=20, font=('Arial', slider_font_size))
        self.peak_threshold_slider.set(5.0)  # Higher default value
        ...
        # Add PVN/SON/Both selection with radio buttons
        ...
        # Run detection button - REMOVED automatic binding, will only run on button click
        self.run_peaks_btn = tk.Button(
            btn_frame,
            text="Detect Peaks",
            font=('Arial', self.font_sizes['button']),
            bg='lightgreen',
            command=self.run_peak_detection_from_tab  # Manually trigger detection
        )
        ...
        self.manual_peak_btn_in_tab = tk.Button(
            btn_frame,
            text="Manual Peak Selection",
            font=('Arial', self.font_sizes['button']),
            bg='lightblue',
        ...
        self.manual_peak_btn_in_tab.pack(side=tk.LEFT, padx=10)
        ...
        # Initialize peak-specific variables if not done in __init__
    ...
    def create_valley_detection_tab(self):
        ...
        self.control_notebook.add(self.valley_tab, text="Valley Detection")  # Tab name
        ...
        # --- Valley Parameter Sliders with HIGHER DEFAULT VALUES---
        # Prominence slider - default set to higher value (8.0 instead of 1.0)
        ...
        self.valley_width_slider = tk.Scale(self.valley_tab, from_=0.0, to=10.0,
                                            resolution=0.1, orient=tk.HORIZONTAL,
                                            length=800, width=20, font=('Arial', slider_font_size))
        self.valley_width_slider.set(2.0)  # Higher default value
        ...
        self.valley_distance_slider = tk.Scale(self.valley_tab, from_=0.0, to=30.0,
                                               resolution=0.5, orient=tk.HORIZONTAL,
                                               length=800, width=20, font=('Arial', slider_font_size))
        self.valley_distance_slider.set(5.0)  # Higher default value
        ...
        # Share the same signal selection variable from peak tab for consistency
        if not hasattr(self, 'signal_selection_var'):
            self.signal_selection_var = tk.StringVar(value="Primary")
        ...
        self.run_valleys_btn = tk.Button(
            ...
            font=('Arial', self.font_sizes['button']),
            bg='lightgreen',
            command=self.run_valley_detection_from_tab  # Manually trigger detection
        )
        ...
        self.manual_valley_btn_in_tab = tk.Button(
            btn_frame,
            text="Manual Valley Selection",
            font=('Arial', self.font_sizes['button']),
            bg='lightblue',
        ...
        self.manual_valley_btn_in_tab.pack(side=tk.LEFT, padx=10)
        ...
        self.show_valleys_var = tk.BooleanVar(value=True)
        self.show_valleys_check_in_tab = tk.Checkbutton(
            options_frame,
            text="Show Valleys",
            variable=self.show_valleys_var,
            font=('Arial', font_size),
        ...
        self.show_valleys_check_in_tab.pack(side=tk.LEFT, padx=5)
        ...
        # Initialize valley-specific variables if not done in __init__
        ...
        # if not hasattr(self, 'valley_feature_annotations'): self.valley_feature_annotations = []
    ...
    def update_peak_display(self):
        ...
        # Display primary peaks if available
        if self.peaks is not None and (self.signal_selection_var.get() == "Primary" or
                                       self.signal_selection_var.get() == "Both"):
            # Display primary peaks
            self._display_peaks_on_signal(
                self.peaks,
                self.processed_signal,
                self.processed_time,
                'red',  # color for primary peaks
                primary=True
            )
        ...
        if hasattr(self, 'secondary_peaks') and self.secondary_peaks is not None and (
                self.signal_selection_var.get() == "Secondary" or
                self.signal_selection_var.get() == "Both"):
            ...
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                self._display_peaks_on_signal(
                    ...
                    'orange',  # different color for secondary peaks
                    primary=False
                )
    ...
    def update_metrics_display(self):
        ...
        if self.signal_selection_var.get() == "Primary" and self.peaks is not None and self.peak_metrics is not None:
            # Display primary peaks
            self._add_peak_metrics_to_tree(self.peaks, self.peak_metrics, "P")

        elif self.signal_selection_var.get() == "Secondary" and hasattr(self,
                                                                        'secondary_peaks') and self.secondary_peaks is not None:
            if hasattr(self, 'secondary_peak_metrics') and self.secondary_peak_metrics is not None:
                # Display secondary peaks
                self._add_peak_metrics_to_tree(self.secondary_peaks, self.secondary_peak_metrics, "S")

        elif self.signal_selection_var.get() == "Both":
            # Display both, with primary first then secondary
            if self.peaks is not None and self.peak_metrics is not None:
                self._add_peak_metrics_to_tree(self.peaks, self.peak_metrics, "P")

            if hasattr(self, 'secondary_peaks') and self.secondary_peaks is not None:
                if hasattr(self, 'secondary_peak_metrics') and self.secondary_peak_metrics is not None:
                    self._add_peak_metrics_to_tree(self.secondary_peaks, self.secondary_peak_metrics, "S")
    ...
    def create_gui(self):
        ...
        self.slider_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=10) # Allow vertical expansion
    ...
    def create_sliders(self):
        ...
        # Add NEW valley detection tab
        self.create_valley_detection_tab()  # <<< ADD THIS LINE
    ...
    def run_valley_detection_from_tab(self, _=None):
        ...
        # Process signals based on the selection radio button
        if self.signal_selection_var.get() == "Both":
            # Process both signals if available
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                ...
                self._run_valley_detection_on_signal(
                    ...
                    "primary",
                    prominence,
                    self.valley_width_slider.get(),
                    self.valley_distance_slider.get()
                )

                # Process secondary signal
                self._run_valley_detection_on_signal(
                    self.secondary_processed_signal,
                    self.secondary_processed_time,
                    "secondary",
                ...
                # Status update showing results for both
                if hasattr(self, 'status_label') and hasattr(self, 'valleys') and hasattr(self, 'secondary_valleys'):
                    prim_count = len(self.valleys['indices']) if self.valleys is not None else 0
                    sec_count = len(self.secondary_valleys['indices']) if self.secondary_valleys is not None else 0
                    self.status_label.config(
                        text=f"Detected {prim_count} PVN valleys and {sec_count} SON valleys",
                        fg="green"
                    )
            else:
                # If secondary is not available, just process primary
                self._run_valley_detection_on_signal(
                    self.processed_signal,
                    self.processed_time,
                    "primary",
        ...
        elif self.signal_selection_var.get() == "Secondary":
            # Only process secondary if available
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                self._run_valley_detection_on_signal(
                    self.secondary_processed_signal,
                    self.secondary_processed_time,
    ...
    def _run_valley_detection_on_signal(self, signal, time_array, signal_type, prominence, width_val, distance_val):
        ...
        if signal_type == "primary":
            ...
            # Create secondary-specific variables
    ...
    def update_valley_display(self):
        """Update the display of ONLY valleys on the plot."""
        ...
        # Exit if no valleys or valleys are hidden
        if self.valleys is None or not self.show_valleys_var.get():
            self.canvas.draw_idle()  # Ensure previous are cleared
            return

        # Determine which signal and time arrays to use (consistency)
        if self.signal_var.get() == "Secondary" and hasattr(self, 'secondary_processed_signal'):
            signal = self.secondary_processed_signal
            time_array = self.secondary_processed_time
        else:
        ...
        valleys_scatter = self.ax1.scatter(
            valley_times_min, self.valleys['depths'],
            marker='v', color='blue', s=100, zorder=10, picker=5
        )
        self.valley_annotations.append(valleys_scatter)  # Add to valley list

        # Add valley labels if enabled
        if self.show_valley_labels_var.get():  # Use valley-specific label var
            for i, (t, d) in enumerate(zip(valley_times_min, self.valleys['depths'])):
                label = self.ax1.annotate(
                    f"V{i + 1}", (t, d), xytext=(0, -15), textcoords="offset points",
                    ha='center', fontsize=9, bbox=dict(boxstyle="round,pad=0.3", fc="white", alpha=0.7)
                )
                self.valley_annotations.append(label)  # Add to valley list

        # Add vertical lines for valleys
        for i, t in enumerate(valley_times_min):
            line = self.ax1.axvline(t, color='blue', linestyle=':', alpha=0.4, zorder=5)  # Dotted line for valleys?
            self.valley_lines.append(line)  # Add to valley list
            ...
            if valid_prec_peak and valid_foll_peak:
                try:
                    ...
                    if segment_indices.size > 1:
                        ...
                        area_val = np.trapz(np.maximum(0, peak_level - segment_signal), segment_time)
    ...
    def create_control_notebook(self):
        """Create the control notebook with tabs"""
        self.control_notebook = ttk.Notebook(self.slider_frame)
        self.control_notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    ...
    def _run_peak_detection_on_signal(self, signal, time_array, signal_type, prominence, width_val, distance_val,
        ...
        peaks, valleys = find_peaks_valleys(
            signal, time_array,
            prominence=prominence, width=width, distance=distance, threshold=threshold
        )
        ...
        if signal_type == "primary":
            self.peaks = peaks
            self.valleys = valleys
            self.peak_metrics = calculate_peak_metrics(peaks, valleys, signal, time_array)
        else:  # "secondary"
            # Create secondary-specific variables if needed
            self.secondary_peaks = peaks
            self.secondary_valleys = valleys
        ...
        # Status update
        if hasattr(self, 'status_label'):
            count = len(peaks['indices']) if peaks is not None else 0
            region = "PVN" if signal_type == "primary" else "SON"
            self.status_label.config(
    ...
    def run_peak_detection_from_tab(self, _=None):
        ...
        # Get PEAK detection parameters from peak tab sliders
        prominence = self.peak_prominence_slider.get()
        threshold = None if self.peak_threshold_slider.get() <= 0 else self.peak_threshold_slider.get()
        ...
        if self.signal_selection_var.get() == "Both":
            ...
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                ...
                self._run_peak_detection_on_signal(
                    ...
                    "primary",
                    prominence,
                    self.peak_width_slider.get(),
                    self.peak_distance_slider.get(),
                    threshold
                )
                ...
                # Status update showing results for both
                if hasattr(self, 'status_label') and hasattr(self, 'peaks') and hasattr(self, 'secondary_peaks'):
                    prim_count = len(self.peaks['indices']) if self.peaks is not None else 0
                    sec_count = len(self.secondary_peaks['indices']) if self.secondary_peaks is not None else 0
                    self.status_label.config(
                        text=f"Detected {prim_count} PVN peaks and {sec_count} SON peaks",
                        fg="green"
                    )
            else:
                # If secondary is not available, just process primary
    ...
    def update_metrics_display(self):
        """Update the PEAK metrics tree view."""
        # Ensure we are targeting the correct tree view
        if not hasattr(self, 'peak_metrics_tree'): return
    ...
    def toggle_manual_peak_mode(self):
        """Toggle manual peak selection mode"""
        self.manual_peak_mode = not self.manual_peak_mode
        self.manual_valley_mode = False  # Ensure valley mode is off

        if self.manual_peak_mode:
            self.manual_peak_btn_in_tab.config(text="Cancel Peak Selection", bg='salmon')
            if hasattr(self, 'manual_valley_btn_in_tab'):  # Disable valley button in other tab
                self.manual_valley_btn_in_tab.config(state=tk.DISABLED, bg='lightgrey')
            self.connect_selection_events("peak")
            if hasattr(self, 'status_label'):
                self.status_label.config(text="Click to add peak. Right-click peak to remove.", fg="blue")
        else:
            self.manual_peak_btn_in_tab.config(text="Manual Peak Selection", bg='lightblue')
            if hasattr(self, 'manual_valley_btn_in_tab'):  # Re-enable valley button
                self.manual_valley_btn_in_tab.config(state=tk.NORMAL, bg='lightblue')
            self.disconnect_selection_events()
            if hasattr(self, 'status_label'):
                self.status_label.config(text="Manual peak selection disabled", fg="green")

    def toggle_manual_valley_mode(self):
        """Toggle manual valley selection mode"""
        self.manual_valley_mode = not self.manual_valley_mode
        self.manual_peak_mode = False  # Ensure peak mode is off

        if self.manual_valley_mode:
            self.manual_valley_btn_in_tab.config(text="Cancel Valley Selection", bg='salmon')
            if hasattr(self, 'manual_peak_btn_in_tab'):  # Disable peak button in other tab
                self.manual_peak_btn_in_tab.config(state=tk.DISABLED, bg='lightgrey')
            self.connect_selection_events("valley")
            if hasattr(self, 'status_label'):
                self.status_label.config(text="Click to add valley. Right-click valley to remove.", fg="blue")
        else:
            self.manual_valley_btn_in_tab.config(text="Manual Valley Selection", bg='lightblue')
            if hasattr(self, 'manual_peak_btn_in_tab'):  # Re-enable peak button
                self.manual_peak_btn_in_tab.config(state=tk.NORMAL, bg='lightblue')
            self.disconnect_selection_events()
            if hasattr(self, 'status_label'):
                self.status_label.config(text="Manual valley selection disabled", fg="green")

    # Modify on_selection_click and on_pick_event to check the mode
    def on_selection_click(self, event, mode):
        """Handle mouse click for adding peaks or valleys based on mode."""
        # Check mode passed from connect_selection_events
        if event.button != 1 or event.inaxes != self.ax1:
            return
        # Check if the correct manual mode is active
        if mode == "peak" and not self.manual_peak_mode: return
        if mode == "valley" and not self.manual_valley_mode: return

        # ... (rest of the existing logic to find index idx) ...
        x_min = event.xdata
        x_sec = x_min * 60
        ...
        if signal is None or time_array is None: return
        idx = np.argmin(np.abs(time_array - x_sec))

        # Initialize if needed (keep this generic)
        if self.peaks is None: self.peaks = {'indices': np.array([], dtype=int), 'times': np.array([]),
                                             'heights': np.array([]), 'properties': {}}
        if self.valleys is None: self.valleys = {'indices': np.array([], dtype=int), 'times': np.array([]),
                                                 'depths': np.array([]), 'properties': {}}
        if self.peak_metrics is None: self.peak_metrics = {'area_under_curve': [], 'full_width_half_max': [],
                                                           'rise_time': [], 'decay_time': [], 'preceding_valley': [],
                                                           'following_valley': []}
        if self.valley_metrics is None: self.valley_metrics = {'area_above_valley': [], 'full_width_half_depth': [],
                                                               'time_to_preceding_peak': [],
                                                               'time_to_following_peak': [], 'preceding_peak_idx': [],
                                                               'following_peak_idx': []}

        if mode == "peak":
            # Add peak
            self.peaks['indices'] = np.append(self.peaks['indices'], idx)
            self.peaks['times'] = np.append(self.peaks['times'], time_array[idx])
            self.peaks['heights'] = np.append(self.peaks['heights'], signal[idx])
            sort_indices = np.argsort(self.peaks['times'])
            self.peaks['indices'] = self.peaks['indices'][sort_indices]
            self.peaks['times'] = self.peaks['times'][sort_indices]
            self.peaks['heights'] = self.peaks['heights'][sort_indices]
            self.status_label.config(text=f"Added peak at {time_array[idx]:.2f}s", fg="green")

            # Recalculate peak metrics
            if len(self.peaks['indices']) > 0 and len(self.valleys['indices']) > 0:
                self.peak_metrics = calculate_peak_metrics(self.peaks, self.valleys, signal, time_array)
                # Also recalc valley metrics as peaks changed context
                self.valley_metrics = self.calculate_valley_metrics(self.peaks, self.valleys, signal, time_array)

            self.update_peak_display()
            self.update_metrics_display()  # Update peak tree
            self.update_valley_metrics_display()  # Update valley tree

        elif mode == "valley":
            # Add valley
            self.valleys['indices'] = np.append(self.valleys['indices'], idx)
            self.valleys['times'] = np.append(self.valleys['times'], time_array[idx])
            self.valleys['depths'] = np.append(self.valleys['depths'], signal[idx])
            sort_indices = np.argsort(self.valleys['times'])
            self.valleys['indices'] = self.valleys['indices'][sort_indices]
            self.valleys['times'] = self.valleys['times'][sort_indices]
            self.valleys['depths'] = self.valleys['depths'][sort_indices]
            self.status_label.config(text=f"Added valley at {time_array[idx]:.2f}s", fg="green")
