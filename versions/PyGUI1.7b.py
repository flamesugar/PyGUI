...
def calculate_df_f(self, signal, reference, baseline_correction=True):
    """
    Calculate ΔF/F based on the selected reference and correction options.

    Parameters:
    -----------
    signal : array
        The target fluorescence signal (e.g., 470nm)
    reference : array or str
        The reference signal (e.g., 410nm) or "Baseline"
    ...
    if baseline_correction:
        # Use iterative weighted least squares for baseline fitting
        signal_copy, baseline_fitted = self.fit_baseline(signal_copy)

        if isinstance(reference, np.ndarray) and reference is not None:
            # Also apply baseline correction to reference signal if it's a numpy array
            reference, _ = self.fit_baseline(reference.copy())

    # Calculate median value for baseline-based calculation
    signal_median = np.median(signal_copy)

    # Calculate ΔF/F based on reference type
    if isinstance(reference, str) and reference == "Baseline":
        # Use signal median as F0
        df_f = 100 * (signal_copy - signal_median) / signal_median
    elif isinstance(reference, np.ndarray) and reference is not None:
        # Use motion correction to get fitted reference
        fitted_reference = self.adaptive_motion_correction_v2(reference, signal_copy)

        # ΔF/F using the fitted reference
        df_f = 100 * (signal_copy - fitted_reference) / fitted_reference
    else:
        # Fallback to simple median-based calculation if no reference provided
        df_f = 100 * (signal_copy - signal_median) / signal_median

    return df_f


def fit_baseline(self, signal, poly_degree=3, num_iterations=3):
    """
    Fit baseline using iterative weighted least squares.
    ...
    num_iterations : int
        Number of iterations for weighted fitting
    ...
    # Create normalized time basis for numerical stability
    x = np.linspace(0, 1, len(signal))

    # Initialize weights to be all equal
    weights = np.ones_like(signal)

    # Iteratively fit polynomial and update weights
    for _ in range(num_iterations):
        # Weighted polynomial fit
        coeffs = np.polynomial.polynomial.polyfit(x, signal, poly_degree, w=weights)
        baseline = np.polynomial.polynomial.polyval(x, coeffs)

        # Calculate residuals
        residuals = signal - baseline

        # Update weights (lower weight for positive residuals)
        # This assumes the baseline is below the signal peaks
        weights = 1.0 / (1.0 + np.exp(10 * residuals / np.std(residuals)))

    # Final baseline fit with updated weights
    coeffs = np.polynomial.polynomial.polyfit(x, signal, poly_degree, w=weights)
    baseline = np.polynomial.polynomial.polyval(x, coeffs)

    # Return corrected signal (original - baseline) and the baseline
    return signal - baseline + np.median(signal), baseline


def adaptive_motion_correction_v2(self, control_signal, target_signal, method="robust"):
    """
    Enhanced version of adaptive motion correction with different fitting methods.

    Parameters:
    -----------
    control_signal : array
        The control signal (typically 405nm or 410nm)
    target_signal : array
        The target signal to be corrected (typically 470nm)
    method : str
        Fitting method: "robust" or "poly"

    Returns:
    --------
    fitted_control : array
        The fitted control signal
    """
    if control_signal is None or target_signal is None:
        return None

    # Ensure equal length
    min_len = min(len(control_signal), len(target_signal))
    control = control_signal[:min_len]
    target = target_signal[:min_len]

    # Normalize signals for better fitting
    control_norm = (control - np.median(control)) / np.std(control)
    target_norm = (target - np.median(target)) / np.std(target)

    from scipy import stats

    if method == "robust":
        # Robust linear fit to reduce impact of outliers (e.g., calcium events)
        try:
            # Try to use RANSAC for robust fitting if scikit-learn is available
            try:
                from sklearn.linear_model import RANSACRegressor, LinearRegression

                # Reshape for scikit-learn
                X = control_norm.reshape(-1, 1)
                y = target_norm

                # Create and fit RANSAC model
                ransac = RANSACRegressor(
                    LinearRegression(),
                    min_samples=int(len(control) * 0.3),
                    max_trials=100,
                    residual_threshold=2.0
                )

                ransac.fit(X, y)
                predicted_norm = ransac.predict(X)
            except (ImportError, ModuleNotFoundError):
                # Fallback if scikit-learn isn't available
                print("scikit-learn not available, using robust linear regression")

                # Sort data for iterative robust fitting
                sort_idx = np.argsort(control_norm)
                sorted_x = control_norm[sort_idx]
                sorted_y = target_norm[sort_idx]

                # Iterative fitting excluding outliers
                inlier_mask = np.ones_like(sorted_x, dtype=bool)
                for _ in range(3):  # 3 iterations
                    ...
                    slope, intercept, _, _, _ = stats.linregress(
                        sorted_x[inlier_mask], sorted_y[inlier_mask])

                    # Predict and compute residuals
                    pred = slope * sorted_x + intercept
                    residuals = sorted_y - pred

                    # Update inlier mask (exclude points with residuals > 2 std)
                    std_resid = np.std(residuals[inlier_mask])
                    inlier_mask = np.abs(residuals) < 2 * std_resid

                # Final fit on inliers
                slope, intercept, _, _, _ = stats.linregress(
                    sorted_x[inlier_mask], sorted_y[inlier_mask])

                # Create prediction for all points
                predicted_norm = slope * control_norm + intercept

            # Convert back to original scale
            predicted = predicted_norm * np.std(target) + np.median(target)

        except Exception as e:
            print(f"Robust fitting failed: {e}, falling back to standard regression")
            # Fallback to standard linear regression
            slope, intercept, _, _, _ = stats.linregress(control_norm, target_norm)
            predicted_norm = slope * control_norm + intercept
            predicted = predicted_norm * np.std(target) + np.median(target)

    elif method == "poly":
        # Polynomial fit for more complex motion relationships
        degree = 3  # Use cubic polynomial
        coeffs = np.polynomial.polynomial.polyfit(control_norm, target_norm, degree)
        predicted_norm = np.polynomial.polynomial.polyval(control_norm, coeffs)
        predicted = predicted_norm * np.std(target) + np.median(target)

    else:
        # Default fallback - simple linear regression
        slope, intercept, _, _, _ = stats.linregress(control_norm, target_norm)
        predicted_norm = slope * control_norm + intercept
        predicted = predicted_norm * np.std(target) + np.median(target)
...
def apply_smoothing(self, signal, method="savgol", window_size=21):
    """
    Apply smoothing to a signal using selected method.
    ...
    signal : array
        Signal to smooth
    method : str
        Method to use ("savgol" or "mavg")
    window_size : int
        Window size for smoothing (must be odd for Savitzky-Golay)
    ...
    if method == "savgol":
        ...
        poly_order = min(3, window_size - 2)  # Ensure valid polynomial order
...
def calculate_z_score(self, signal):
    """
    Calculate Z-score normalization of a signal.
...
def calculate_mad(self, signal, start_pct=0, end_pct=30):
    """
    Calculate the Median Absolute Deviation for a specific interval.
    ...
    signal : array
        Signal to analyze
    start_pct : float
        Start of interval as percentage of signal length
    end_pct : float
        End of interval as percentage of signal length
    ...
    start_idx = int(len(signal) * start_pct / 100)
    end_idx = int(len(signal) * end_pct / 100)
    ...
    interval = signal[start_idx:end_idx]

    # Calculate median
    median = np.median(interval)

    # Calculate MAD
    mad = np.median(np.abs(interval - median))

    return mad


def detect_peaks(self, signal, time, mad_threshold=3.0, mad=None):
    """
    Detect peaks in the signal that exceed a threshold based on MAD.
    ...
    mad_threshold : float
        Threshold as a multiple of MAD
    mad : float, optional
        Precalculated MAD value (will calculate if None)

    Returns:
    --------
    peaks : list
        List of dictionaries with peak data (time, height, width)
    """
    if signal is None or time is None or len(signal) < 3:
        return []

    # Calculate MAD if not provided
    if mad is None:
        mad = self.calculate_mad(signal,
                                 self.mad_start_var.get(),
                                 self.mad_end_var.get())
    ...
    while i < len(signal) - 1:
        # Check if the point is higher than both neighbors and exceeds threshold
        if (signal[i] > signal[i - 1] and signal[i] > signal[i + 1] and
                signal[i] - min(signal[i - 1], signal[i + 1]) > threshold):

            # Found a peak candidate - now determine the peak base
            # Find left and right limits where signal drops below peak
            left_idx = i
            while left_idx > 0 and signal[left_idx] > signal[left_idx - 1]:
                left_idx -= 1

            right_idx = i
            while right_idx < len(signal) - 1 and signal[right_idx] > signal[right_idx + 1]:
                right_idx += 1

            # Calculate peak properties
            peak_time = time[i]
            peak_height = signal[i] - min(signal[left_idx], signal[right_idx])
            peak_width = time[right_idx] - time[left_idx]
            ...
            # Skip to after this peak to avoid counting it multiple times
            i = right_idx
...
def detect_and_mark_peaks(self):
    """
    ...
    if signal is None or time is None:
        messagebox.showinfo("Error", "No signal data available")
        return

    # Get MAD settings
    mad_start = self.mad_start_var.get()
    mad_end = self.mad_end_var.get()
    mad_threshold = self.mad_threshold_var.get()

    # Calculate MAD
    mad = self.calculate_mad(signal, mad_start, mad_end)

    # Detect peaks
    peaks = self.detect_peaks(signal, time, mad_threshold, mad)

    # Clear any existing peak markers
    for artist in self.ax1.get_children():
        if isinstance(artist, plt.Line2D) and hasattr(artist, 'is_peak_marker'):
            artist.remove()

    # No peaks found
    if not peaks:
        messagebox.showinfo("Peaks", "No peaks detected with current settings.")
        self.canvas.draw_idle()
        return

    # Mark peaks on plot
    for peak in peaks:
        # Peak marker (red dot at peak)
        peak_line = self.ax1.plot(
            peak['time'] / 60,  # Convert to minutes for display
            signal[peak['peak_idx']],
            'ro',
            markersize=8,
            alpha=0.7
        )[0]
        peak_line.is_peak_marker = True

        # Peak height line
        height_line = self.ax1.plot(
            [peak['time'] / 60, peak['time'] / 60],
            [signal[peak['left_idx']], signal[peak['peak_idx']]],
            'r-',
            linewidth=1.5,
            alpha=0.7
        )[0]
        height_line.is_peak_marker = True

    # Redraw canvas
    self.canvas.draw_idle()

    # Show summary
    messagebox.showinfo("Peaks", f"Detected {len(peaks)} peaks.\nMAD: {mad:.4f}")


def update_display_mode(self):
    """
    Update the display mode between ΔF/F and Z-score
    """
    ...
    # Store the original processed signal if not already stored
    if not hasattr(self, 'original_processed_signal'):
        self.original_processed_signal = self.processed_signal.copy()

    if mode == "zscore":
        # Convert to Z-score
        z_score = self.calculate_z_score(self.original_processed_signal)
        self.processed_signal = z_score

        # Update y-axis label
        self.ax1.set_ylabel('Z-Score', fontsize=self.font_sizes['base'])
    else:
        # Restore original dF/F signal
        self.processed_signal = self.original_processed_signal.copy()
    ...
    # Same for secondary signal if available
    if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
        if not hasattr(self, 'original_secondary_processed_signal'):
            self.original_secondary_processed_signal = self.secondary_processed_signal.copy()

        if mode == "zscore":
            z_score = self.calculate_z_score(self.original_secondary_processed_signal)
            self.secondary_processed_signal = z_score
        else:
            self.secondary_processed_signal = self.original_secondary_processed_signal.copy()
...
class PhotometryViewer:
    ...
    def create_sliders(self):
        ...
        # Add this to the create_sliders() method after creating the other tabs
        self.motion_smooth_tab = ttk.Frame(self.control_notebook)
        self.control_notebook.add(self.motion_smooth_tab, text="Motion & Smooth")

        # Configure grid
        self.motion_smooth_tab.columnconfigure(1, weight=1)

        # Section for ΔF/F calculation
        df_frame = tk.LabelFrame(self.motion_smooth_tab, text="ΔF/F Calculation",
                                 font=('Arial', font_size, 'bold'))
        df_frame.grid(row=0, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        # Reference selection
        self.reference_var = tk.StringVar(value="410")
        tk.Label(df_frame, text="Reference signal:", font=('Arial', font_size)).grid(
            row=0, column=0, sticky="w", padx=10, pady=5)
        tk.Radiobutton(df_frame, text="410nm", variable=self.reference_var,
                       value="410", font=('Arial', font_size),
                       command=self.update_filter).grid(row=0, column=1, sticky="w", padx=10)
        tk.Radiobutton(df_frame, text="560nm", variable=self.reference_var,
                       value="560", font=('Arial', font_size),
                       command=self.update_filter).grid(row=0, column=2, sticky="w", padx=10)
        tk.Radiobutton(df_frame, text="Baseline", variable=self.reference_var,
                       value="Baseline", font=('Arial', font_size),
                       command=self.update_filter).grid(row=0, column=3, sticky="w", padx=10)

        # Baseline correction checkbox
        self.baseline_correction_var = tk.BooleanVar(value=True)
        tk.Checkbutton(df_frame, text="Apply baseline correction",
                       variable=self.baseline_correction_var, font=('Arial', font_size),
                       command=self.update_filter).grid(row=1, column=0, columnspan=4, sticky="w", padx=10, pady=5)

        # Motion correction section
        motion_frame = tk.LabelFrame(self.motion_smooth_tab, text="Motion Correction",
                                     font=('Arial', font_size, 'bold'))
        motion_frame.grid(row=1, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        self.motion_correction_var = tk.BooleanVar(value=True)
        tk.Checkbutton(motion_frame, text="Apply motion correction",
                       variable=self.motion_correction_var, font=('Arial', font_size),
                       command=self.update_filter).grid(row=0, column=0, sticky="w", padx=10, pady=5)

        # Motion correction method
        self.motion_method_var = tk.StringVar(value="robust")
        tk.Label(motion_frame, text="Method:", font=('Arial', font_size)).grid(
            row=1, column=0, sticky="w", padx=10, pady=5)
        tk.Radiobutton(motion_frame, text="Robust Linear Fit", variable=self.motion_method_var,
                       value="robust", font=('Arial', font_size),
                       command=self.update_filter).grid(row=1, column=1, sticky="w", padx=10)
        tk.Radiobutton(motion_frame, text="Polynomial", variable=self.motion_method_var,
                       value="poly", font=('Arial', font_size),
                       command=self.update_filter).grid(row=1, column=2, sticky="w", padx=10)

        # Smoothing section
        smooth_frame = tk.LabelFrame(self.motion_smooth_tab, text="Smoothing",
                                     font=('Arial', font_size, 'bold'))
        smooth_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        self.smoothing_var = tk.BooleanVar(value=True)
        tk.Checkbutton(smooth_frame, text="Apply smoothing",
                       variable=self.smoothing_var, font=('Arial', font_size),
                       command=self.update_filter).grid(row=0, column=0, sticky="w", padx=10, pady=5)

        # Smoothing method
        self.smooth_method_var = tk.StringVar(value="savgol")
        tk.Label(smooth_frame, text="Method:", font=('Arial', font_size)).grid(
            row=1, column=0, sticky="w", padx=10, pady=5)
        tk.Radiobutton(smooth_frame, text="Savitzky-Golay", variable=self.smooth_method_var,
                       value="savgol", font=('Arial', font_size),
                       command=self.update_filter).grid(row=1, column=1, sticky="w", padx=10)
        tk.Radiobutton(smooth_frame, text="Moving Average", variable=self.smooth_method_var,
                       value="mavg", font=('Arial', font_size),
                       command=self.update_filter).grid(row=1, column=2, sticky="w", padx=10)

        # Smoothing window size
        tk.Label(smooth_frame, text="Window size:", font=('Arial', font_size)).grid(
            row=2, column=0, sticky="w", padx=10, pady=5)
        self.smooth_window_var = tk.IntVar(value=21)
        tk.Spinbox(smooth_frame, from_=3, to=101, increment=2, textvariable=self.smooth_window_var,
                   width=5, command=self.update_filter).grid(row=2, column=1, sticky="w", padx=10)

        # MAD settings for peak detection
        mad_frame = tk.LabelFrame(self.motion_smooth_tab, text="MAD Settings",
                                  font=('Arial', font_size, 'bold'))
        mad_frame.grid(row=3, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        # MAD interval selection
        tk.Label(mad_frame, text="MAD interval selection:", font=('Arial', font_size)).grid(
            row=0, column=0, sticky="w", padx=10, pady=5)

        # Add sliders for selecting MAD interval
        tk.Label(mad_frame, text="Start (%):", font=('Arial', font_size)).grid(
            row=1, column=0, sticky="w", padx=10, pady=5)
        self.mad_start_var = tk.DoubleVar(value=0.0)
        tk.Scale(mad_frame, from_=0.0, to=100.0, resolution=1.0, orient=tk.HORIZONTAL,
                 variable=self.mad_start_var, length=slider_length // 2, width=15,
                 font=('Arial', slider_font_size - 1)).grid(
            row=1, column=1, sticky="w", padx=10, pady=5)

        tk.Label(mad_frame, text="End (%):", font=('Arial', font_size)).grid(
            row=2, column=0, sticky="w", padx=10, pady=5)
        self.mad_end_var = tk.DoubleVar(value=30.0)
        tk.Scale(mad_frame, from_=0.0, to=100.0, resolution=1.0, orient=tk.HORIZONTAL,
                 variable=self.mad_end_var, length=slider_length // 2, width=15,
                 font=('Arial', slider_font_size - 1)).grid(
            row=2, column=1, sticky="w", padx=10, pady=5)

        # MAD threshold for peak detection
        tk.Label(mad_frame, text="Peak threshold (×MAD):", font=('Arial', font_size)).grid(
            row=3, column=0, sticky="w", padx=10, pady=5)
        self.mad_threshold_var = tk.DoubleVar(value=3.0)
        tk.Scale(mad_frame, from_=0.01, to=20.0, resolution=0.01, orient=tk.HORIZONTAL,
                 variable=self.mad_threshold_var, length=slider_length // 2, width=15,
                 font=('Arial', slider_font_size - 1)).grid(
            row=3, column=1, sticky="w", padx=10, pady=5)

        # Display mode selection
        display_frame = tk.LabelFrame(self.motion_smooth_tab, text="Display Mode",
                                      font=('Arial', font_size, 'bold'))
        display_frame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        self.display_mode_var = tk.StringVar(value="dff")
        tk.Radiobutton(display_frame, text="ΔF/F (%)", variable=self.display_mode_var,
                       value="dff", font=('Arial', font_size),
                       command=self.update_display_mode).grid(row=0, column=0, sticky="w", padx=10)
        tk.Radiobutton(display_frame, text="Z-Score", variable=self.display_mode_var,
                       value="zscore", font=('Arial', font_size),
                       command=self.update_display_mode).grid(row=0, column=1, sticky="w", padx=10)

        # Button to detect and mark peaks
        self.detect_peaks_btn = tk.Button(
            self.motion_smooth_tab,
            text="Detect and Mark Peaks",
            font=('Arial', self.font_sizes['button']),
            command=self.detect_and_mark_peaks
        )
        self.detect_peaks_btn.grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def update_filter(self, _=None, preserve_blanking=False):
        try:
            ...
            # Check if we have the motion & smooth settings defined
            if hasattr(self, 'motion_correction_var'):
                # Apply motion correction if enabled
                if self.motion_correction_var.get() and self.downsampled_raw_analog_2 is not None:
                    # Get method selection
                    motion_method = self.motion_method_var.get()

                    # Apply motion correction
                    fitted_control = self.adaptive_motion_correction_v2(
                        self.downsampled_raw_analog_2,
                        processed_signal,
                        method=motion_method
                    )

                    # Apply motion correction if successful
                    if fitted_control is not None:
                        # Motion corrected = original - (fitted_control - mean(fitted_control))
                        motion_component = fitted_control - np.mean(fitted_control)
                        processed_signal = processed_signal - motion_component

                        # If there's a secondary file, also apply to it
                        if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                            if hasattr(self,
                                       'secondary_downsampled_raw_analog_2') and self.secondary_downsampled_raw_analog_2 is not None:
                                secondary_fitted = self.adaptive_motion_correction_v2(
                                    self.secondary_downsampled_raw_analog_2,
                                    self.secondary_processed_signal,
                                    method=motion_method
                                )
                                if secondary_fitted is not None:
                                    motion_comp = secondary_fitted - np.mean(secondary_fitted)
                                    self.secondary_processed_signal = self.secondary_processed_signal - motion_comp

                # Apply smoothing if enabled
                if hasattr(self, 'smoothing_var') and self.smoothing_var.get():
                    # Get smoothing parameters
                    smooth_method = self.smooth_method_var.get()
                    smooth_window = self.smooth_window_var.get()

                    # Apply smoothing
                    processed_signal = self.apply_smoothing(
                        processed_signal,
                        method=smooth_method,
                        window_size=smooth_window
                    )

                    # Also apply to secondary if available
                    if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                        self.secondary_processed_signal = self.apply_smoothing(
                            self.secondary_processed_signal,
                            method=smooth_method,
                            window_size=smooth_window
                        )

                # Apply dF/F calculation if settings exist
                if hasattr(self, 'reference_var'):
                    reference_type = self.reference_var.get()
                    baseline_correction = self.baseline_correction_var.get()

                    # Select appropriate reference signal
                    if reference_type == "410":
                        reference_signal = self.downsampled_raw_analog_2
                    elif reference_type == "560":
                        # If secondary file is loaded and is 560nm data
                        if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                            reference_signal = self.secondary_downsampled_raw_analog_1
                        else:
                            reference_signal = None
                    else:  # Baseline
                        reference_signal = "Baseline"

                    # Calculate dF/F for primary signal
                    dff_signal = self.calculate_df_f(
                        processed_signal,
                        reference_signal,
                        baseline_correction
                    )

                    if dff_signal is not None:
                        processed_signal = dff_signal

                        # Store original for possible Z-score calculation later
                        self.original_processed_signal = processed_signal.copy()

                        # Check if we want Z-score instead of dF/F
                        if hasattr(self, 'display_mode_var') and self.display_mode_var.get() == "zscore":
                            processed_signal = self.calculate_z_score(processed_signal)

                            # Update y-axis label
                            if hasattr(self, 'ax1'):
                                self.ax1.set_ylabel('Z-Score', fontsize=self.font_sizes['base'])
                        else:
                            # Ensure y-axis label is correct
                            if hasattr(self, 'ax1'):
                                self.ax1.set_ylabel('% ΔF/F', fontsize=self.font_sizes['base'])
