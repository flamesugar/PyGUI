...
class PhotometryViewer:
    ...
    def on_scroll(self, event):
        """
        Targeted fix for Ctrl+Scroll X-only zoom.
        """
        # print(f"on_scroll triggered. Ctrl: {self.ctrl_pressed}, Button: {event.button}, Axes: {event.inaxes}, XDATA: {event.xdata}, YDATA: {event.ydata}")
        if event.inaxes is None or not hasattr(self, 'canvas') or self.canvas is None:
            return False

        base_scale = 1.2
        if event.button == 'up':  # Scroll up/away from user = zoom in
            zoom_factor = 1.0 / base_scale
        elif event.button == 'down':  # Scroll down/towards user = zoom out
        ...
        if self.ctrl_pressed:
            # === CTRL+WHEEL: Zoom ONLY X-axis of ALL panels, centered on mouse X position ===
            print(f"Ctrl+Scroll: X-Zoom. Factor: {zoom_factor:.2f}.")

            # Use x-limits from the current axes the mouse is over as the reference
            current_xlim = ax_current.get_xlim()
            xdata = event.xdata

            if xdata is None or not np.isfinite(xdata):
                xdata = (current_xlim[0] + current_xlim[1]) / 2

            new_left = xdata - (xdata - current_xlim[0]) * zoom_factor
            new_right = xdata + (current_xlim[1] - xdata) * zoom_factor

            if new_right <= new_left or abs(new_right - new_left) < 1e-9:  # Prevent collapse
                print("  Ctrl+Scroll: X-limit calculation resulted in too small/invalid range. Aborting zoom.")
                return False

            print(f"  Target X-limits: [{new_left:.3f}, {new_right:.3f}]")

            # Explicitly store and restore Y limits for ALL relevant axes
            axes_to_update = [self.ax1, self.ax2, self.ax3]
            original_y_lims = {}
            for ax_panel in axes_to_update:
                if ax_panel:
                    original_y_lims[ax_panel] = ax_panel.get_ylim()
                    ax_panel.set_autoscaley_on(False)  # Crucial: disable Y autoscaling

            # Apply X-limits
            # Since they share X, setting it on one should propagate, but let's be explicit
            for ax_panel in axes_to_update:
                if ax_panel:
                    ax_panel.set_xlim(new_left, new_right)

            # Force restore Y-limits AFTER X-limits are set
            for ax_panel in axes_to_update:
                if ax_panel and ax_panel in original_y_lims:
                    ax_panel.set_ylim(original_y_lims[ax_panel])
                    # print(f"  Panel {ax_panel}: X set, Y restored to {original_y_lims[ax_panel]}")

        else:
            # === REGULAR WHEEL: Zoom Y-axis of CURRENT panel ONLY (ax1 or ax2) ===
            if ax_current not in [self.ax1, self.ax2]:  # Skip ax3 (digital) and others
                return False

            print(f"Regular Scroll: Y-Zoom on {ax_current}. Factor: {zoom_factor:.2f}.")
            current_ylim = ax_current.get_ylim()
            ydata = event.ydata
            if ydata is None or not np.isfinite(ydata):
                ydata = (current_ylim[0] + current_ylim[1]) / 2

            new_bottom = ydata - (ydata - current_ylim[0]) * zoom_factor
            new_top = ydata + (current_ylim[1] - ydata) * zoom_factor

            if new_top <= new_bottom or abs(new_top - new_bottom) < 1e-9:  # Prevent collapse
                print("  Regular Scroll: Y-limit calculation resulted in too small/invalid range. Aborting zoom.")
                return False

            original_x_lim = ax_current.get_xlim()
            ax_current.set_autoscalex_on(False)  # Crucial: disable X autoscaling for this axis

            ax_current.set_ylim(new_bottom, new_top)
            ax_current.set_xlim(original_x_lim)  # Restore X
            # print(f"  Panel {ax_current}: Y set to [{new_bottom:.3f},{new_top:.3f}], X restored.")

        self.canvas.draw_idle()
        return True  # Try to consume the event fully
