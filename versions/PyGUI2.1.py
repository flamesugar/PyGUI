...
def comprehensive_photometry_processing(signal_470, control_405, time, sampling_rate=300.0):
    ...
    try:
        # Validate inputs
        if signal_470 is None or len(signal_470) == 0:
            print("Error: Signal data is empty")
            return None
        ...
        if sampling_rate is None or sampling_rate <= 0:
            ...
            if time is not None and len(time) > 1:
                ...
                print(f"Calculated sampling rate: {sampling_rate} Hz")
            else:
                sampling_rate = 300.0  # Default
        ...
        def butterworth_lowpass(signal, cutoff=10.0, fs=sampling_rate, order=2):
            ...
            nyq = 0.5 * fs
            normal_cutoff = min(0.95, cutoff / nyq)  # Ensure cutoff is valid

            try:
                sos = butter(order, normal_cutoff, btype='low', analog=False, output='sos')
                return sosfiltfilt(sos, signal)
            except Exception as e:
                print(f"Butterworth filter error: {e}")
                return signal
        ...
        try:
            ...
            print(f"Error in control scaling: {e}")
            # Safe fallback if scaling fails
            scale_factor = 1.0
            offset = np.mean(filtered_470) - np.mean(filtered_405)
            scaled_405 = filtered_405 + offset
            print(f"Using fallback scaling with offset={offset:.4f}")
        ...
        try:
            ...
            motion_corrected = filtered_470  # Use uncorrected signal as fallback
        ...
        try:
            ...
            print(f"Error in artifact removal: {e}")
            artifact_cleaned = motion_corrected  # Use uncleaned signal as fallback
        ...
        try:
            ...
            poly_degree = min(3, len(time_normalized) // 1000)  # Limit based on data length
            ...
            detrended = artifact_cleaned  # Use uncleaned signal as fallback
            bleaching_trend = np.zeros_like(artifact_cleaned)
        ...
        try:
            # Calculate appropriate window size (must be odd and > poly_order)
            poly_order = 3

            # Calculate minimum window size based on sampling rate (0.5-1 seconds of data)
            min_window_points = max(5, int(sampling_rate * 0.5))

            # Ensure window size is larger than poly_order and odd
            window_size = max(poly_order + 2, min_window_points)
            if window_size % 2 == 0:  # Ensure odd
                window_size += 1

            print(f"Using Savitzky-Golay filter with window_size={window_size}, poly_order={poly_order}")
            ...
            try:
                ...
                window_size = max(5, int(sampling_rate * 0.5))
                smoothed = uniform_filter1d(detrended, size=window_size)
                print(f"Using fallback uniform filter with window_size={window_size}")
            except Exception as e2:
                print(f"Fallback filter also failed: {e2}")
                smoothed = detrended  # Use unsmoothed signal as final fallback
        ...
        try:
            ...
            # Make sure baseline is not zero or very close to zero
            if abs(f0) < 1e-6:
                f0 = np.mean(smoothed) / 100  # Use a small fraction of the mean
                print(f"Adjusted zero baseline to {f0}")

            # Calculate ΔF/F
            dff = (smoothed - f0) / f0 * 100  # Convert to percent
        except Exception as e:
            print(f"Error calculating dF/F: {e}")
            # Fallback method if standard calculation fails
            mean_val = np.mean(smoothed)
            if abs(mean_val) < 1e-6:
                mean_val = 1.0  # Avoid division by zero
            dff = (smoothed / mean_val - 1) * 100
            print("Using fallback dF/F calculation")
        ...
        try:
            ...
            # Provide basic metrics even if binning fails
            result['bins'] = []
            result['metrics'] = {
                'mean_dff': np.mean(dff),
                'max_dff': np.max(dff),
        ...
        print(f"Major error in comprehensive_photometry_processing: {e}")
        import traceback
        traceback.print_exc()

        # Return minimal result with raw data
        return {
            'time': time,
            'raw_470': signal_470,
            'raw_405': control_405,
...
def wavelet_denoise_signal(signal, time, wavelet='sym4', level=None, threshold_multiplier=1.0,
                           progress_callback=None, cancel_event=None):
    """Apply wavelet-based denoising to a signal"""
    import pywt  # Import inside function to handle potential import error gracefully
        ...
        if len(denoised_signal) != len(signal):
            ...
            if len(denoised_signal) < len(signal):
                ...
                denoised_signal = np.pad(denoised_signal,
                                         (0, len(signal) - len(denoised_signal)),
                                         'constant', constant_values=(denoised_signal[-1],))
...
class PhotometryViewer:
    ...
    def add_secondary_data_to_plots(self):
        """Add the secondary file data to the existing plots with comprehensive error handling"""
        try:
            print("Adding secondary data to plots")
            ...
            if not hasattr(self, 'secondary_line') or self.secondary_line is None:
                ...
                print("Created secondary line")
            else:
                print("Updating existing secondary line")
            ...
            if not hasattr(self, 'secondary_raw_line') or self.secondary_raw_line is None:
                ...
                self.secondary_raw_line.set_label(f'{region_label} Analog 1')
            ...
            if self.secondary_downsampled_raw_analog_2 is not None:
                if not hasattr(self, 'secondary_raw_line2') or self.secondary_raw_line2 is None:
                    ...
                    self.secondary_raw_line2.set_label(f'{region_label} Analog 2')

                # Add artifact markers on the secondary 405nm channel
                if not hasattr(self, 'secondary_artifact_markers_raw') or self.secondary_artifact_markers_raw is None:
            ...
            # Clear existing digital lines to prevent duplicates
            for line in self.secondary_digital_lines:
            ...
            if hasattr(self, 'status_label'):
                self.status_label.config(text=f"Error displaying secondary data: {str(e)}", fg="red")
    ...
    def create_gui(self):
        try:
            ...
            # Create navigation frame - ensure it exists before references
            self.nav_frame = tk.Frame(self.main_frame)
            self.nav_frame.pack(fill=tk.X, padx=20, pady=10)

            # Create a frame for the matplotlib figure (ONLY ONCE)
            ...
            # Create GridSpec for subplots - ensure this is included
            ...
            # Auto-scale y-axis with asymmetric scaling
            ...
            # NOW call connect_zoom_events after canvas is created
    ...
    def create_sliders(self):
        ...
        # Add this inside the advanced_denoising_tab section
        self.denoise_method_var = tk.StringVar(value="standard")
        method_frame = tk.LabelFrame(self.advanced_denoising_tab,
                                     text="Denoising Method",
                                     font=('Arial', self.font_sizes['base']))
        method_frame.grid(row=3, column=0, columnspan=2, sticky="ew", padx=10, pady=5)

        tk.Radiobutton(method_frame,
                       text="Standard Denoising",
                       variable=self.denoise_method_var,
                       value="standard",
                       font=('Arial', self.font_sizes['base'])).pack(anchor="w", padx=5, pady=2)

        tk.Radiobutton(method_frame,
                       text="Wavelet Denoising",
                       variable=self.denoise_method_var,
                       value="wavelet",
                       font=('Arial', self.font_sizes['base'])).pack(anchor="w", padx=5, pady=2)

        # Add this to your existing advanced_denoising_tab UI creation
        self.wavelet_btn = tk.Button(
            self.advanced_denoising_tab,
            text="Run Wavelet Denoising",
            font=('Arial', self.font_sizes['button']),
            bg='lightblue',
            height=2,
            command=self.run_wavelet_denoising
        )
        self.wavelet_btn.grid(row=10, column=0, columnspan=2, pady=10)  # Use an available row

        # Add wavelet-specific options
        wavelet_options_frame = tk.Frame(method_frame)
        wavelet_options_frame.pack(fill=tk.X, padx=5, pady=5)

        tk.Label(wavelet_options_frame,
                 text="Wavelet Type:",
                 font=('Arial', self.font_sizes['base'])).grid(row=0, column=0, sticky="w", padx=5, pady=2)

        self.wavelet_type_var = tk.StringVar(value="sym4")
        wavelet_options = ttk.Combobox(wavelet_options_frame,
                                       textvariable=self.wavelet_type_var,
                                       values=["sym4", "db4", "coif3", "haar"],
                                       width=10)
        wavelet_options.grid(row=0, column=1, padx=5, pady=2)

        tk.Label(wavelet_options_frame,
                 text="Threshold:",
                 font=('Arial', self.font_sizes['base'])).grid(row=1, column=0, sticky="w", padx=5, pady=2)

        self.wavelet_threshold_var = tk.DoubleVar(value=1.0)
        tk.Scale(wavelet_options_frame,
                 from_=0.5, to=3.0,
                 resolution=0.1,
                 orient=tk.HORIZONTAL,
                 length=200,
                 variable=self.wavelet_threshold_var).grid(row=1, column=1, padx=5, pady=2)
    ...
    def run_wavelet_denoising(self):
        """Run wavelet denoising with improved error handling and range selection"""
        if hasattr(self, 'denoise_thread') and self.denoise_thread and self.denoise_thread.is_alive():
            messagebox.showinfo("Denoising", "Denoising is already running!")
            return

        # Find a valid parent for progress UI elements
        parent = self.advanced_denoising_tab  # Use directly instead of search logic

        # Create progress UI elements with grid instead of pack
        self.progress_var = tk.DoubleVar()
        ...
        try:
            ...
            # Create new progress bar with grid
            self.progress_bar = ttk.Progressbar(
                parent,
                variable=self.progress_var,
                maximum=100,
                length=400)
            self.progress_bar.grid(row=12, column=0, columnspan=2, pady=10)  # Use grid instead of pack

            # Similarly handle cancel button
            ...
            self.cancel_denoise_button = tk.Button(
                parent,
                text="Cancel Denoising",
                font=('Arial', getattr(self, 'font_sizes', {}).get('base', 12)),
                command=self.cancel_denoising)
            self.cancel_denoise_button.grid(row=13, column=0, columnspan=2, pady=5)  # Use grid

            # Disable run button
            self.wavelet_btn.config(state=tk.DISABLED, text="Wavelet Denoising Running...")
            ...
            # Start the wavelet denoising thread
            self.denoise_thread = threading.Thread(
                target=self.wavelet_denoising_worker
            )
            ...
            print(f"Error starting denoising: {e}")
            if hasattr(self, 'wavelet_btn'):
                self.wavelet_btn.config(state=tk.NORMAL, text="Run Wavelet Denoising")
            messagebox.showerror("Error", f"Failed to start denoising: {str(e)}")

    def setup_wavelet_controls(self):
        """Create enhanced controls for wavelet denoising"""
        wavelet_frame = tk.LabelFrame(self.advanced_denoising_tab, text="Wavelet Denoising Settings")
        wavelet_frame.grid(row=11, column=0, columnspan=2, sticky="ew", padx=10, pady=10)

        # Range selection
        range_frame = tk.Frame(wavelet_frame)
        range_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=5)

        tk.Label(range_frame, text="Apply to range:").grid(row=0, column=0, sticky="w", padx=5)

        # Range selector
        self.wavelet_range_var = tk.BooleanVar(value=False)
        tk.Checkbutton(range_frame, text="Use selected range only",
                       variable=self.wavelet_range_var).grid(row=0, column=1, sticky="w")

        # Start time
        tk.Label(range_frame, text="Start time (s):").grid(row=1, column=0, sticky="w", padx=5)
        self.wavelet_start_var = tk.DoubleVar(value=0.0)
        tk.Entry(range_frame, textvariable=self.wavelet_start_var, width=8).grid(row=1, column=1, sticky="w")

        # End time
        tk.Label(range_frame, text="End time (s):").grid(row=2, column=0, sticky="w", padx=5)
        self.wavelet_end_var = tk.DoubleVar(value=60.0)
        tk.Entry(range_frame, textvariable=self.wavelet_end_var, width=8).grid(row=2, column=1, sticky="w")

        # Enhanced wavelet parameters
        param_frame = tk.Frame(wavelet_frame)
        param_frame.grid(row=3, column=0, columnspan=2, sticky="ew", pady=5)

        # Wavelet type with more options
        tk.Label(param_frame, text="Wavelet Type:").grid(row=0, column=0, sticky="w", padx=5)
        wavelet_options = ["sym4", "sym8", "db4", "db8", "coif3", "haar", "dmey"]
        self.wavelet_type_var = tk.StringVar(value="sym4")
        tk.OptionMenu(param_frame, self.wavelet_type_var, *wavelet_options).grid(row=0, column=1, sticky="w")

        # Decomposition level
        tk.Label(param_frame, text="Decomposition Level:").grid(row=1, column=0, sticky="w", padx=5)
        self.wavelet_level_var = tk.IntVar(value=0)  # 0 means auto
        level_frame = tk.Frame(param_frame)
        level_frame.grid(row=1, column=1, sticky="w")
        tk.Radiobutton(level_frame, text="Auto", variable=self.wavelet_level_var, value=0).pack(side=tk.LEFT)
        for i in range(1, 6):
            tk.Radiobutton(level_frame, text=str(i), variable=self.wavelet_level_var, value=i).pack(side=tk.LEFT)

        # Threshold multiplier with finer control
        tk.Label(param_frame, text="Threshold Strength:").grid(row=2, column=0, sticky="w", padx=5)
        self.wavelet_threshold_var = tk.DoubleVar(value=1.0)
        tk.Scale(param_frame, variable=self.wavelet_threshold_var,
                 from_=0.1, to=3.0, resolution=0.05, orient=tk.HORIZONTAL,
                 length=200).grid(row=2, column=1, sticky="w")

        # Preview button
        preview_btn = tk.Button(wavelet_frame, text="Preview Effect",
                                command=self.preview_wavelet_denoising)
        preview_btn.grid(row=4, column=0, pady=10)

        # Apply button (moved from create_sliders)
        self.wavelet_btn = tk.Button(
            wavelet_frame,
            text="Apply Wavelet Denoising",
            font=('Arial', self.font_sizes['button']),
            bg='lightblue',
            command=self.run_wavelet_denoising
        )
        self.wavelet_btn.grid(row=4, column=1, pady=10)

    def wavelet_denoising_worker(self):
        """Worker function for wavelet denoising with range selection and better error handling"""
        try:
            # Initialize secondary_denoised with None to prevent UnboundLocalError
            secondary_denoised = None

            # Get parameters with safe defaults
            wavelet_type = getattr(self, 'wavelet_type_var', tk.StringVar(value='sym4')).get()
            threshold = float(getattr(self, 'wavelet_threshold_var', tk.DoubleVar(value=1.0)).get())

            # Check if we're using a blanking selection for range
            use_selection = False
            range_start = None
            range_end = None

            # Check for existing blanking selection
            if hasattr(self, 'blanking_selection') and self.blanking_selection is not None:
                use_selection = True
                range_start, range_end = self.blanking_selection
            ...
            if hasattr(self, 'processed_signal') and self.processed_signal is not None:
                try:
                    ...
                    self.denoise_progress_queue.put(
                        ('status', f"Denoising primary signal with {wavelet_type} wavelet..."))

                    # Apply to selected range or whole signal
                    if use_selection:
                        # Find indices for the blanking range
                        start_idx = np.argmin(np.abs(self.processed_time - range_start))
                        end_idx = np.argmin(np.abs(self.processed_time - range_end))

                        # Make copy of signal
                        primary_denoised = self.processed_signal.copy()

                        # Apply wavelet to just the selected range
                        segment = self.processed_signal[start_idx:end_idx + 1]
                        segment_time = self.processed_time[start_idx:end_idx + 1]

                        denoised_segment = wavelet_denoise_signal(
                            segment,
                            segment_time,
                        ...
                        # Replace segment in full signal
                        if denoised_segment is not None:
                            primary_denoised[start_idx:end_idx + 1] = denoised_segment

                        # Clear blanking selection
                        self.clear_blanking_selection()

                    else:
                        # Run wavelet denoising on the full signal
                        primary_denoised = wavelet_denoise_signal(
                            self.processed_signal,
                            self.processed_time,
                            wavelet=wavelet_type,
                            threshold_multiplier=threshold,
                    ...
                    if primary_denoised is None:
                        print("Wavelet denoising returned None for primary signal")
                        primary_denoised = self.processed_signal.copy()

                except Exception as e:
                    print(f"Error in primary wavelet denoising: {e}")
                    import traceback
                    traceback.print_exc()
                    primary_denoised = self.processed_signal.copy()  # Use original on error
                    self.denoise_progress_queue.put(('status', f"Error with primary signal: {str(e)[:50]}..."))
            ...
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                try:
                    ...
                    if not self.cancel_denoise_event.is_set():
                        # Update progress message
                        self.denoise_progress_queue.put(('status', "Denoising secondary signal with wavelets..."))

                        # Apply to selected range or whole signal
                        if use_selection:
                            # Find indices for the blanking range
                            start_idx = np.argmin(np.abs(self.secondary_processed_time - range_start))
                            end_idx = np.argmin(np.abs(self.secondary_processed_time - range_end))

                            # Make copy of signal
                            secondary_denoised = self.secondary_processed_signal.copy()

                            # Apply wavelet to just the selected range
                            segment = self.secondary_processed_signal[start_idx:end_idx + 1]
                            segment_time = self.secondary_processed_time[start_idx:end_idx + 1]
                            ...
                            # Replace segment in full signal
                            if denoised_segment is not None:
                                secondary_denoised[start_idx:end_idx + 1] = denoised_segment
                        else:
                            # Run wavelet on whole signal
                            secondary_denoised = wavelet_denoise_signal(
                                self.secondary_processed_signal,
                                self.secondary_processed_time,
                                wavelet=wavelet_type,
                                threshold_multiplier=threshold,
                        ...
                        if secondary_denoised is None:
                            print("Wavelet denoising returned None for secondary signal")
                            secondary_denoised = self.secondary_processed_signal.copy()

                except Exception as e:
                    print(f"Error in secondary wavelet denoising: {e}")
                    import traceback
                    traceback.print_exc()
                    secondary_denoised = self.secondary_processed_signal.copy()  # Use original on error
                    self.denoise_progress_queue.put(('status', f"Error with secondary signal: {str(e)[:50]}..."))

            # Apply edge protection to results if enabled
            if hasattr(self, 'edge_protection_var') and self.edge_protection_var.get():
                try:
                    if primary_denoised is not None:
                        primary_denoised = self.process_edge_effects(primary_denoised, self.processed_time)
                    if secondary_denoised is not None:
                        secondary_denoised = self.process_edge_effects(secondary_denoised,
                                                                       self.secondary_processed_time)
                except Exception as e:
                    print(f"Error in edge protection: {e}")
                    # Continue with unprotected signals
            ...
            # If a major error occurred, send it to the main thread
    ...
    def connect_zoom_events(self):
        """Connect mouse wheel events for custom zooming using an alternative approach"""
        ...
        # Also bind to the Tkinter widget as a backup
    ...
    def _on_mousewheel(self, event):
        """Tkinter event handler for mousewheel with improved error handling"""
        try:
            # Convert to matplotlib-style event direction
            direction = 'up' if event.delta > 0 else 'down'

            # Create a synthetic matplotlib event with safer initialization
            class SyntheticEvent:
                def __init__(self, owner):
                    self.button = direction
                    # Get ax1 from the owner instance rather than itself
                    self.inaxes = getattr(owner, 'ax1', None)
                    self.xdata = None
                    self.ydata = None

            # Create event with reference to self (the PhotometryViewer instance)
            synthetic_event = SyntheticEvent(self)

            # Only proceed if we have a valid axes reference
            if synthetic_event.inaxes is not None:
                self.on_scroll(synthetic_event)
            else:
                print("Warning: Cannot access plot axes for scrolling")
        except Exception as e:
            print(f"Error in mousewheel handler: {e}")

    def _on_ctrl_mousewheel(self, event):
        """Tkinter event handler for Ctrl+mousewheel"""
    ...
    def on_scroll(self, event):
        """
        Handle scroll events (Based on PyGUI2.0.py):
        - Normal scroll: zoom y-axis of the plot under the mouse.
        - Ctrl+scroll: zoom x-axis of all plots, centered on mouse.
        Returns 'break' to potentially stop event propagation.
        """
        try:
            ...
            base_scale = 1.2  # Sensitivity factor (increase for faster zoom)
            if event.button == 'up':
                zoom_factor = 1.0 / base_scale  # Zoom in
            elif event.button == 'down':
                zoom_factor = base_scale  # Zoom out
            else:
                return  # Ignore other scroll buttons if any

            ax_current = event.inaxes  # The specific axes where the scroll happened

            if self.ctrl_pressed:
                # --- X-axis zoom (Ctrl + Scroll) ---
                xlim = ax_current.get_xlim()
                # Center zoom on mouse x-coordinate, or center of view if mouse is outside
                xdata = event.xdata if event.xdata is not None and np.isfinite(event.xdata) else (xlim[0] + xlim[1]) / 2

                new_width = (xlim[1] - xlim[0]) * zoom_factor
                # Calculate new limits relative to the zoom center
                new_xlim = [xdata - new_width * (xdata - xlim[0]) / (xlim[1] - xlim[0]),
                            xdata + new_width * (xlim[1] - xdata) / (xlim[1] - xlim[0])]

                # Prevent zooming out beyond reasonable limits (optional, based on PyGUI2.0.py)
                if zoom_factor > 1.0 and hasattr(self, 'time') and self.time is not None and len(self.time) > 1:
                    full_range = (self.time[-1] - self.time[0]) / 60  # Full range in minutes
                    margin = full_range * 0.05  # 5% margin
                    min_time = self.time[0] / 60 - margin
                    max_time = self.time[-1] / 60 + margin
                    if new_xlim[0] < min_time: new_xlim[0] = min_time
                    if new_xlim[1] > max_time: new_xlim[1] = max_time

                # Apply limits to all relevant axes
                for ax in [self.ax1, self.ax2, self.ax3]:
                    if ax: ax.set_xlim(new_xlim)

            else:
                # --- Y-axis zoom (Normal Scroll) ---
                if ax_current == self.ax3:
                    return  # Skip Y-zoom on the digital plot (ax3)

                ylim = ax_current.get_ylim()
                # Center zoom on mouse y-coordinate, or center of view if mouse is outside
                ...
                # Apply limits only to the axis where the scroll occurred
                ax_current.set_ylim(new_ylim)

            # Redraw the canvas to show the changes
            self.canvas.draw_idle()
            print(
                f"Scroll event received: button={event.button}, inaxes={event.inaxes}, ctrl_pressed={self.ctrl_pressed}")

            # Attempt to stop the event from propagating to parent Tkinter widgets
            # This might prevent the main window or other frames from scrolling
            return 'break'

        except Exception as e:
            # Log any errors during the scroll event handling
            print(f"Error during scroll event processing: {e}")
            import traceback
            traceback.print_exc()
            # It's often good practice to still return 'break' or None even on error
            return 'break'
    ...
    def run_advanced_denoising(self):
        ...
        try:
            # Process primary file
            has_primary_artifacts = hasattr(self, 'artifact_mask') and self.artifact_mask is not None and np.any(
                self.artifact_mask)

            # Process secondary file if available
            has_secondary_artifacts = False
            if hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None:
                has_secondary_artifacts = hasattr(self,
                                                  'secondary_artifact_mask') and self.secondary_artifact_mask is not None and np.any(
                    self.secondary_artifact_mask)
            ...
            # Recreate progress bar to ensure it exists in the current context
            if hasattr(self, 'progress_bar'):
                try:
                    self.progress_bar.destroy()
                except Exception as e:
                    print(f"Couldn't destroy progress bar: {e}")
                    pass

            # Create a new progress bar in the correct parent
            self.progress_var = tk.DoubleVar()

            # Ensure the parent exists and is viable
            if not hasattr(self, 'advanced_denoising_tab') or self.advanced_denoising_tab is None:
                print("Warning: advanced_denoising_tab not found, creating progress bar in root")
                self.progress_bar = ttk.Progressbar(
                    self.root,
                    variable=self.progress_var,
                    maximum=100,
            ...
            if hasattr(self, 'cancel_denoise_button'):
                try:
                    self.cancel_denoise_button.destroy()
                except Exception as e:
                    print(f"Couldn't destroy cancel button: {e}")
                    pass

            # Create the cancel button in the appropriate parent
            if not hasattr(self, 'advanced_denoising_tab') or self.advanced_denoising_tab is None:
                self.cancel_denoise_button = tk.Button(
                    self.root,
                    text="Cancel Denoising",
                    font=('Arial', self.font_sizes['base'] if hasattr(self, 'font_sizes') else 12),
                    command=self.cancel_denoising
                )
            else:
                self.cancel_denoise_button = tk.Button(
            ...
            # Reset cancellation event
            if not hasattr(self, 'cancel_denoise_event'):
                self.cancel_denoise_event = threading.Event()
            else:
                self.cancel_denoise_event.clear()

            # Initialize or clear queue
            if not hasattr(self, 'denoise_progress_queue'):
                self.denoise_progress_queue = queue.Queue()
            else:
                # Clear queue
                while not self.denoise_progress_queue.empty():
            ...
            print(f"Error starting advanced denoising: {e}")
            import traceback
            traceback.print_exc()
            ...
            # Clean up any partially created UI
            if hasattr(self, 'progress_bar'):
                try:
                    self.progress_bar.grid_remove()
                except:
            ...
            # Inform user
            messagebox.showerror("Denoising Error", f"Failed to start denoising: {str(e)}")
    ...
    def denoise_both_files_worker(self):
        ...
        try:
            ...
            # Get the denoising method
            denoise_method = self.denoise_method_var.get()

            # Additional parameters for wavelet denoising
            wavelet_type = self.wavelet_type_var.get()
            threshold_multiplier = self.wavelet_threshold_var.get()
            ...
            if self.artifact_mask is not None and np.any(self.artifact_mask):
                ...
                # Choose denoising method based on user selection
                if denoise_method == "wavelet":
                    ...
                    # Use the existing advanced_denoise_signal
    ...
    def check_denoise_progress(self):
        ...
        try:
            ...
            if not self.denoise_thread.is_alive():
                ...
                # Reset all denoising buttons to normal state
                if hasattr(self, 'denoise_button'):
                    self.denoise_button.config(state=tk.NORMAL, text="Run Advanced Denoising")
                ...
                # Update status
                if hasattr(self, 'status_label'):
                    self.status_label.config(text="Denoising completed", fg="green")
    ...
    # In create_sliders() method, add this section to the advanced_denoising_tab
    def modify_advanced_denoising_tab(self):
        """Add wavelet denoising controls that work with blanking selection"""

        # Add this instruction label
        blanking_wavelet_label = tk.Label(
            self.advanced_denoising_tab,
            text="To apply wavelet denoising to a specific range:\n"
                 "1. Enable Selection Mode and select a region\n"
                 "2. Click 'Run Wavelet Denoising'",
            font=('Arial', self.font_sizes['base'] - 1),
            fg='blue')
        blanking_wavelet_label.grid(row=11, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        # Wavelet type and threshold controls
        wavelet_frame = tk.Frame(self.advanced_denoising_tab)
        wavelet_frame.grid(row=13, column=0, columnspan=2, padx=10, pady=5, sticky="ew")

        tk.Label(
            wavelet_frame,
            text="Wavelet Type:",
            font=('Arial', self.font_sizes['base'])).grid(row=0, column=0, sticky="w", padx=5)

        self.wavelet_type_var = tk.StringVar(value="sym4")
        wavelet_combo = ttk.Combobox(
            wavelet_frame,
            textvariable=self.wavelet_type_var,
            values=["sym4", "db4", "coif3", "haar", "sym8", "db8", "dmey"],
            width=8)
        wavelet_combo.grid(row=0, column=1, sticky="w", padx=5)

        tk.Label(
            wavelet_frame,
            text="Threshold Strength:",
            font=('Arial', self.font_sizes['base'])).grid(row=1, column=0, sticky="w", padx=5)

        self.wavelet_threshold_var = tk.DoubleVar(value=1.0)
        tk.Scale(
            wavelet_frame,
            variable=self.wavelet_threshold_var,
            from_=0.1, to=3.0,
            resolution=0.1,
            orient=tk.HORIZONTAL,
            length=200).grid(row=1, column=1, sticky="w", padx=5)
        ...
        self.wavelet_btn = tk.Button(
            ...
            height=2,
            command=self.run_wavelet_denoising)
        self.wavelet_btn.grid(row=14, column=0, columnspan=2, pady=10)
