...
class PhotometryViewer:
    ...
    def run_denoise_for_artifacts(self):
        """Run denoising specifically on detected artifacts"""
        # Check if artifacts are detected
        ...
        if not has_primary_artifacts and not has_secondary_artifacts:
            messagebox.showinfo("Denoising", "No artifacts detected! Please adjust the artifact threshold.")
            return
        ...
        # Disable run button
        self.denoise_button.config(state=tk.DISABLED, text="Denoising in progress...")
        ...
        if hasattr(self, 'status_label'):
            total_artifacts = 0
            if has_primary_artifacts:
                total_artifacts += np.sum(self.artifact_mask)
            if has_secondary_artifacts:
                total_artifacts += np.sum(self.secondary_artifact_mask)

            self.status_label.config(
                text=f"Denoising {total_artifacts} artifacts. Please wait...",
                fg="blue"
            )
