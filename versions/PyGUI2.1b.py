...
class PhotometryViewer:
    ...
    def _on_mousewheel(self, event):
        ...
        # This is a simplification and might need adjustment based on actual panel sizes
    ...
    def on_scroll(self, event):
        """
        Enhanced scroll handler with panel-specific zooming:
        - When scrolling over panel 1 or 2, only zoom that panel's y-axis
        - Ctrl+scroll still zooms x-axis for all panels together
        """
        try:
            ...
            # Debug which panel is being zoomed
            panel_name = "unknown"
            if ax_current == self.ax1:
                panel_name = "panel1 (ax1)"
            elif ax_current == self.ax2:
                panel_name = "panel2 (ax2)"
            elif ax_current == self.ax3:
                panel_name = "panel3 (ax3)"
            print(f"Scroll event on {panel_name}, ctrl_pressed={self.ctrl_pressed}")

            if self.ctrl_pressed:
                # === X-axis zoom (Ctrl + Scroll) - affects all panels ===
                xlim = ax_current.get_xlim()
                # Center zoom on mouse x-coordinate, or center of view if mouse is outside data range
                ...
                # Apply limits to all axes that share the x-axis
                for ax in [self.ax1, self.ax2, self.ax3]:
                    if ax: ax.set_xlim(new_xlim)

                print(f"Applied X-axis zoom to all panels: {new_xlim}")
            else:
                # === Y-axis zoom - ONLY affects the panel being scrolled ===
                ...
                print(f"Applied Y-axis zoom to panel {panel_name}: {new_ylim}")

            # Redraw the canvas to show the changes
            self.canvas.draw_idle()

            # Attempt to stop the event from propagating
            return False
