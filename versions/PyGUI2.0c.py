...
def get_font_size_for_resolution(self):
...
class PhotometryViewer:
    def __init__(self, root, file_path=None):
        ...
        # Define font sizes directly for initialization
        screen_width = root.winfo_screenwidth()
        if screen_width >= 3840:  # 4K or higher
            self.font_sizes = {
                'base': 22,  # Significantly increased
                'button': 20,  # Increased
                'title': 26,  # Increased
                'slider': 18  # Increased
            }
        ...
        # Add variables to track plot window
        self.plot_window = None
        self.plot_window_visible = False

        # Initialize processing parameters
        self.low_cutoff = 0.001
    ...
    def load_file(self, file_path):
        ...
        if isinstance(result, tuple) and len(result) == 9:
            ...
            self.drift_curve = None  # Set drift_curve to None if not returned
        elif isinstance(result, tuple) and len(result) == 10:
            # 10-value return (includes drift_curve)
            self.processed_time, self.processed_signal, self.processed_signal_no_control, \
                self.processed_analog_2, self.downsampled_raw_analog_1, self.downsampled_raw_analog_2, \
                self.processed_digital_1, self.processed_digital_2, self.artifact_mask, \
                self.drift_curve = result
        else:
            print(f"Warning: process_data returned unexpected format: {type(result)}")
            return

        # Update plot elements if they exist, otherwise create them
        ...
        # Update status
        if hasattr(self, 'status_label'):
            self.status_label.config(text=f"Loaded file: {os.path.basename(file_path)}", fg="green")

    def update_plot_elements(self):
        """Update or create plot elements based on loaded data"""
        # First update font sizes
        ...
        if self.line_no_control is None:
        ...
        if self.drift_curve is not None and self.drift_correction:
            ...
            if self.drift_line is None:
                ...
                self.drift_line.set_ydata(df_drift_curve)

        # Initialize artifact markers
        if self.artifact_markers_processed is None:
        ...
        if self.downsampled_raw_analog_2 is not None:
            ...
            if self.artifact_markers_raw is None:
                self.artifact_markers_raw, = self.ax2.plot(
                    [], [], 'ro', ms=4, alpha=0.7,
                    label=f'{primary_region} Artifacts'
                )

        # Clear any existing digital lines
        for line in self.digital_lines:
            line.remove()
        self.digital_lines = []

        # Add new connection
        self._pick_cid = self.fig.canvas.mpl_connect('pick_event', self.on_legend_pick)
        ...
        # Recreate legends to ensure they're visible and properly sized
        self.ax1_legend = self.create_checkbox_legend(self.ax1)
        self.ax2_legend = self.create_checkbox_legend(self.ax2)
        if self.digital_lines:
            self.ax3_legend = self.create_checkbox_legend(self.ax3)
    ...
    def add_secondary_data_to_plots(self):
        ...
        # Make sure the plot window is visible
        self.plot_window.deiconify()
        self.toggle_plot_btn.config(text="Hide Plots", bg='salmon')
    ...
    def create_checkbox_legend(self, ax):
        ...
        if not lines:  # If no lines are found, return None
            return None
        ...
        if screen_width >= 3840:  # 4K
            legend_fontsize = self.font_sizes['base'] * 1.5
        else:
            legend_fontsize = self.font_sizes['base']

        # Create legend with custom handler
        leg = ax.legend(loc='upper right', fontsize=legend_fontsize)
        ...
        for i, legline in enumerate(leg.get_lines()):
            ...
            # Make legend lines thicker and more visible
            legline.set_linewidth(3)

        # Increase text sizes in legend
        for text in leg.get_texts():
            text.set_fontsize(legend_fontsize)

        # Connect the pick event to the right figure
    ...
    def update_font_sizes(self):
        """Update font sizes for all plot elements based on screen resolution"""
        # Get screen dimensions
        screen_width = self.root.winfo_screenwidth()

        # Calculate scaling factor for high-DPI screens
        if screen_width >= 3840:  # 4K
            dpi_factor = 1.5
        else:
            dpi_factor = 1.0
        ...
        if hasattr(self, 'fig') and self.fig is not None:
            plt.rcParams.update({
                'font.size': int(self.font_sizes['base'] * dpi_factor),
                'axes.titlesize': int(self.font_sizes['title'] * dpi_factor),
                'axes.labelsize': int(self.font_sizes['base'] * dpi_factor),
                'xtick.labelsize': int(self.font_sizes['base'] * dpi_factor),
                'ytick.labelsize': int(self.font_sizes['base'] * dpi_factor),
                'legend.fontsize': int(self.font_sizes['base'] * dpi_factor),
            })

            # Update axis labels
            for ax in [self.ax1, self.ax2, self.ax3]:
                if ax is not None:
                    ax.set_title(ax.get_title(), fontsize=int(self.font_sizes['title'] * dpi_factor))
                    ax.set_xlabel(ax.get_xlabel(), fontsize=int(self.font_sizes['base'] * dpi_factor))
                    ax.set_ylabel(ax.get_ylabel(), fontsize=int(self.font_sizes['base'] * dpi_factor))
                    ax.tick_params(labelsize=int(self.font_sizes['base'] * dpi_factor))

            # Update legends
            for legend in [self.ax1_legend, self.ax2_legend, self.ax3_legend]:
                if legend is not None:
                    for text in legend.get_texts():
                        text.set_fontsize(int(self.font_sizes['base'] * dpi_factor))
    ...
    def on_legend_pick(self, event):
        ...
        # Make sure this is a legend line pick event
        if hasattr(event.artist, 'line'):
    ...
    def create_gui(self):
        ...
        # Create the plot window but don't show it yet
    ...
    def create_plot_window(self):
        """Create the detachable plot window with controls"""
        # Create a new top-level window for plots
        self.plot_window = tk.Toplevel(self.root)
        self.plot_window.title("Signal Viewer")
        ...
        # Set window size (80% of screen for 4K)
        if screen_width >= 3840:  # 4K
            window_width = int(screen_width * 0.8)
            window_height = int(screen_height * 0.8)
            dpi_factor = 1.5  # Higher DPI factor for 4K
        else:
            window_width = int(screen_width * 0.7)
            window_height = int(screen_height * 0.7)
            dpi_factor = 1.0

        self.plot_window.geometry(f"{window_width}x{window_height}")

        # Position window
        x_position = min(self.root.winfo_x() + self.root.winfo_width(), screen_width - window_width)
        y_position = self.root.winfo_y()
        self.plot_window.geometry(f"+{x_position}+{y_position}")

        # Create navigation panel at the top of plot window
        self.plot_nav_frame = tk.Frame(self.plot_window)
        self.plot_nav_frame.pack(fill=tk.X, padx=10, pady=5)

        # Add navigation buttons to the plot window
        self.add_plot_navigation_buttons()

        # Create frame for the matplotlib figure
        self.frame = tk.Frame(self.plot_window)
        self.frame.pack(fill=tk.BOTH, expand=True)

        # Create matplotlib figure with higher DPI for 4K screens
        dpi = 120 * dpi_factor  # Increase DPI for better visibility
        self.fig = plt.figure(figsize=(10, 8), dpi=dpi)

        # Configure matplotlib for better high-DPI display
        plt.rcParams.update({
            'font.size': int(self.font_sizes['base'] * dpi_factor),
            'axes.titlesize': int(self.font_sizes['title'] * dpi_factor),
            'axes.labelsize': int(self.font_sizes['base'] * dpi_factor),
        ...
        gs = gridspec.GridSpec(3, 1, height_ratios=[5, 3, 1])  # Reduced height for 3rd panel

        # Panel 1: Processed signal plot
        self.ax1 = self.fig.add_subplot(gs[0])
        self.ax1.set_ylabel('% ΔF/F', fontsize=int(self.font_sizes['base'] * dpi_factor))
        self.ax1.set_title('Processed Photometry Signal', fontsize=int(self.font_sizes['title'] * dpi_factor))
        self.ax1.grid(True, alpha=0.3)
        self.ax1.tick_params(labelsize=int(self.font_sizes['base'] * dpi_factor))

        # Panel 2: Raw signals plot
        self.ax2 = self.fig.add_subplot(gs[1], sharex=self.ax1)
        self.ax2.set_ylabel('mV', fontsize=int(self.font_sizes['base'] * dpi_factor))
        self.ax2.set_title('Raw Analog Signals', fontsize=int(self.font_sizes['title'] * dpi_factor))
        self.ax2.grid(True, alpha=0.3)
        self.ax2.tick_params(labelsize=int(self.font_sizes['base'] * dpi_factor))

        # Panel 3: Digital signals (smaller)
        self.ax3 = self.fig.add_subplot(gs[2], sharex=self.ax1)
        self.ax3.set_ylabel('TTL', fontsize=int(self.font_sizes['base'] * dpi_factor))
        self.ax3.set_ylim(-0.1, 1.1)
        self.ax3.set_xlabel('Time (minutes)', fontsize=int(self.font_sizes['base'] * dpi_factor))
        self.ax3.tick_params(labelsize=int(self.font_sizes['base'] * dpi_factor))
        ...
        # Hide the plot window initially
        self.plot_window.withdraw()

    def add_plot_navigation_buttons(self):
        """Add navigation buttons to the plot window"""
        # Get screen width for responsive sizing
        screen_width = self.root.winfo_screenwidth()

        # Scale button width and font size for high-DPI screens
        if screen_width >= 3840:  # 4K
            btn_width = 12
            btn_font_size = self.font_sizes['button'] + 4
        else:
            btn_width = 10
            btn_font_size = self.font_sizes['button']

        button_font = ('Arial', btn_font_size)

        # Create buttons in the plot navigation frame
        hide_btn = tk.Button(
            self.plot_nav_frame,
            text="Hide Plots",
            font=button_font,
            width=btn_width,
            bg='salmon',
            command=lambda: self.toggle_plot_window()
        )
        hide_btn.pack(side=tk.LEFT, padx=5)

        # Zoom buttons
        zoom_x_in_btn = tk.Button(
            self.plot_nav_frame,
            text="Zoom In (X)",
            font=button_font,
            width=btn_width,
            command=lambda: self.zoom_x(in_out="in")
        )
        zoom_x_in_btn.pack(side=tk.LEFT, padx=5)

        zoom_x_out_btn = tk.Button(
            self.plot_nav_frame,
            text="Zoom Out (X)",
            font=button_font,
            width=btn_width,
            command=lambda: self.zoom_x(in_out="out")
        )
        zoom_x_out_btn.pack(side=tk.LEFT, padx=5)

        # Y axis zoom buttons
        zoom_y_in_btn = tk.Button(
            self.plot_nav_frame,
            text="Zoom In (Y)",
            font=button_font,
            width=btn_width,
            command=lambda: self.zoom_y_all("in")
        )
        zoom_y_in_btn.pack(side=tk.LEFT, padx=5)

        zoom_y_out_btn = tk.Button(
            self.plot_nav_frame,
            text="Zoom Out (Y)",
            font=button_font,
            width=btn_width,
            command=lambda: self.zoom_y_all("out")
        )
        zoom_y_out_btn.pack(side=tk.LEFT, padx=5)
        ...
        pan_left_btn = tk.Button(
            ...
            font=button_font,
            width=btn_width,
            command=lambda: self.pan_x(direction="left")
        )
        pan_left_btn.pack(side=tk.LEFT, padx=5)
        ...
        pan_right_btn = tk.Button(
            ...
            font=button_font,
            width=btn_width,
            command=lambda: self.pan_x(direction="right")
        )
        pan_right_btn.pack(side=tk.LEFT, padx=5)
        ...
        primary_btn.pack(side=tk.LEFT, padx=5)
        ...
        secondary_btn = tk.Button(
            ...
            width=btn_width,
            bg='lightgreen',
            command=self.open_secondary_file
        )
        ...
        clear_btn = tk.Button(
            ...
            font=button_font,
            width=btn_width,
            command=self.clear_secondary_file
        )
        clear_btn.pack(side=tk.LEFT, padx=5)

    def on_plot_window_close(self):
        """Handle the plot window being closed by hiding it instead of destroying"""
        self.plot_window.withdraw()  # Hide the window instead of closing it
        # You might want to add a button to show it again
    ...
    def add_navigation_buttons(self):
        ...
        # Add Toggle Plot Window button
        self.toggle_plot_btn = tk.Button(
            self.nav_frame,
            text="Show/Hide Plots",
            font=title_font,
            width=15,
            command=self.toggle_plot_window,
            bg='lightgreen'
        )
        self.toggle_plot_btn.grid(row=0, column=1, padx=5, pady=5)
    ...
    def toggle_plot_window(self):
        """Toggle the visibility of the plot window"""
        if hasattr(self, 'plot_window'):
            if self.plot_window.winfo_viewable():
                self.plot_window.withdraw()
                self.toggle_plot_btn.config(text="Show Plots", bg='lightgreen')
            else:
                ...
                if hasattr(self, 'processed_signal') and self.processed_signal is not None:
                    ...
                    messagebox.showinfo("No Data", "Please load a data file first")
        else:
            messagebox.showinfo("Not Ready", "Plot window has not been initialized yet")
    ...
    def update_filter(self, _=None, preserve_blanking=True):
        ...
        try:
            ...
            if isinstance(result, tuple) and len(result) == 9:
                ...
                print(
                    f"Warning: process_data returned unexpected format: {type(result)} with {len(result) if hasattr(result, '__len__') else 'unknown'} values")
                return
            ...
            if hasattr(self, 'secondary_file_path') and self.secondary_file_path:
                ...
                # Handle different return value counts for secondary data
                if isinstance(secondary_result, tuple) and len(secondary_result) == 9:
                    self.secondary_processed_time, self.secondary_processed_signal, self.secondary_processed_signal_no_control, \
                        self.secondary_processed_analog_2, self.secondary_downsampled_raw_analog_1, self.secondary_downsampled_raw_analog_2, \
                        self.secondary_processed_digital_1, self.secondary_processed_digital_2, self.secondary_artifact_mask = secondary_result
                    self.secondary_drift_curve = None
                elif isinstance(secondary_result, tuple) and len(secondary_result) == 10:
                    self.secondary_processed_time, self.secondary_processed_signal, self.secondary_processed_signal_no_control, \
                        self.secondary_processed_analog_2, self.secondary_downsampled_raw_analog_1, self.secondary_downsampled_raw_analog_2, \
                        self.secondary_processed_digital_1, self.secondary_processed_digital_2, self.secondary_artifact_mask, \
                        self.secondary_drift_curve = secondary_result

            # Continue with the rest of your update_filter method...
            ...
            if hasattr(self, 'plot_window') and self.plot_window is not None:
                ...
                if hasattr(self, 'line') and self.line is not None:
                    ...
                    # Update drift line if available
