...
class PhotometryViewer:
    ...
    def open_baseline_alignment_window(self):
        ...
        tk.Radiobutton(align_frame, text="Simple Alignment",
                       variable=self.alignment_mode_var,
                       value="simple").pack(anchor="w")
        tk.Radiobutton(align_frame, text="Advanced Alignment (for complex signals)",
                       variable=self.alignment_mode_var,
                       value="advanced").pack(anchor="w")

        # Sensitivity slider
        sensitivity_frame = tk.Frame(align_frame)
        sensitivity_frame.pack(fill=tk.X, pady=10)

        tk.Label(sensitivity_frame, text="Detection Sensitivity:").pack(side=tk.LEFT)
        self.sensitivity_var = tk.DoubleVar(value=1.0)
        tk.Scale(sensitivity_frame, from_=0.5, to=2.0, resolution=0.1,
        ...
        tk.Button(btn_frame, text="Apply Alignment",
    ...
    def _on_mousewheel(self, event):
        ...
        class SyntheticEvent:
            def __init__(self, outer_self):
                ...
                self.inaxes = outer_self.ax1  # Reference the PhotometryViewer's ax1
                self.xdata = None
                self.ydata = None

        # Pass self to the SyntheticEvent constructor
        self.on_scroll(SyntheticEvent(self))
