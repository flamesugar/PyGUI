...
def auto_align_baselines(signal, time, threshold_pct=3.0, window_size=None, smoothing_size=101, transition_size=100):
    ...
    window_size : int or None
        Window size for baseline estimation (default: auto based on signal length)
    smoothing_size : int
        Window size for low-pass filtering to estimate baseline (must be odd)
    transition_size : int
    ...
    # Ensure odd smoothing size (required for savgol_filter)
    if smoothing_size % 2 == 0:
        smoothing_size += 1

    # Set window size if not provided
    if window_size is None:
        window_size = max(int(len(signal) * 0.03), 50)  # Default to 3% of signal length or 50 points

    # Get sample rate to calculate time-based windows
    dt = np.mean(np.diff(time))
    fs = 1.0 / dt
    window_time = window_size / fs

    print(f"Using window size of {window_size} points ({window_time:.2f} sec)")

    # 1. Smooth the signal to estimate baseline (low frequency component)
    # Using Savitzky-Golay filter for better preservation of peaks/valleys
    try:
        baseline_est = savgol_filter(signal, smoothing_size, 1)
    except:
        # Fall back to simple moving average if savgol fails
        from scipy.ndimage import uniform_filter1d
        baseline_est = uniform_filter1d(signal, size=smoothing_size)

    # 2. Calculate derivative of the baseline to find sudden changes
    baseline_deriv = np.diff(baseline_est, prepend=baseline_est[0])

    # Calculate adaptive threshold based on signal characteristics
    deriv_abs = np.abs(baseline_deriv)
    deriv_median = np.median(deriv_abs)
    deriv_mad = np.median(np.abs(deriv_abs - deriv_median))  # Median absolute deviation

    # Threshold for significant change (robust to outliers)
    if deriv_mad > 0:
        # Use median + scaled MAD (robust version of standard deviation)
        adaptive_threshold = deriv_median + 5.0 * deriv_mad  # 5 sigma threshold
    else:
        # Fallback if MAD is zero (very stable signal)
        adaptive_threshold = np.percentile(deriv_abs, 99)

    # Ensure threshold isn't too small (noise) or too large (missing shifts)
    signal_range = np.percentile(signal, 95) - np.percentile(signal, 5)
    min_threshold = signal_range * threshold_pct / 100.0  # Min threshold from % of signal range

    # Use the larger of adaptive or minimum threshold
    final_threshold = max(adaptive_threshold, min_threshold)

    print(f"Shift detection threshold: {final_threshold:.5f}")

    # 3. Find points where derivative exceeds threshold (potential shift points)
    potential_shifts = np.where(deriv_abs > final_threshold)[0]

    # Group adjacent points to avoid multiple detections of the same shift
    shift_points = []
    if len(potential_shifts) > 0:
        current_group = [potential_shifts[0]]

        for i in range(1, len(potential_shifts)):
            if potential_shifts[i] - potential_shifts[i - 1] < window_size // 4:
                # Add to current group if close
                current_group.append(potential_shifts[i])
            else:
                # Add median of current group as a shift point
    ...
    all_points = [0] + shift_points + [len(signal)]
    segments = list(zip(all_points[:-1], all_points[1:]))

    print(f"Detected {len(shift_points)} baseline shifts at indices: {shift_points}")

    # 4. Calculate baseline level for each segment
    baselines = []
    for start, end in segments:
        # Get segment with some margin
        seg_start = max(0, start)
        seg_end = min(len(signal), end)
        segment = signal[seg_start:seg_end]

        # Use percentile instead of mean for better robustness to outliers
        # 10th percentile works well for signals with mostly upward transients
        if len(segment) > 0:
    ...
    # 5. Apply corrections to align baselines
    if len(baselines) > 1:
        target_baseline = baselines[0]  # Use first segment as reference

        for i, ((start, end), baseline) in enumerate(zip(segments[1:], baselines[1:])):
            if i + 1 < len(segments):
                offset = baseline - target_baseline

                # Apply offset to this segment
                if abs(offset) > min_threshold:  # Only correct meaningful shifts
                    print(f"Applying offset of {offset:.5f} to segment {i + 1} ({start}-{end})")

                    # Get transition size (but don't exceed segment size)
                    trans_size = min(transition_size, (end - start) // 3)
                    ...
                    if trans_size > 0:
                        weights = np.linspace(0, 1, trans_size)
                        for j in range(trans_size):
                            aligned_signal[start + j] -= offset * weights[j]
...
def advanced_baseline_alignment(signal, time, window_size=None,
    ...
    time : array
        Corresponding time points
    window_size : int or None
        Size of window for baseline estimation (default: auto)
    segmentation_sensitivity : float
        Controls sensitivity of shift detection (higher = more sensitive)
    use_changepoint_detection : bool
        Whether to use statistical changepoint detection (more robust for complex signals)
    ...
    if window_size is None:
        # Use 2% of signal length but between 30 and 500 points
        window_size = max(30, min(500, int(len(signal) * 0.02)))

    # Make a copy for output
    aligned_signal = signal.copy()

    # Step 1: Detect changepoints using robust segmentation
    if use_changepoint_detection:
        try:
            # Try to use ruptures package if available (pip install ruptures)
            import ruptures as rpt

            # Apply filtering to reduce noise
            smoothed_signal = savgol_filter(signal, 51, 3)

            # Use Binary Segmentation for changepoint detection
            algo = rpt.Binseg(model="l2").fit(smoothed_signal.reshape(-1, 1))

            # Automatically determine number of segments based on sensitivity
            n_segments = max(2, int(10 * segmentation_sensitivity))
            changepoints = algo.predict(n_bkps=n_segments - 1)
            ...
            print(f"Changepoint detection library not available or error: {e}")
            print("Using manual segmentation instead...")
            changepoints = []
            use_changepoint_detection = False

    # Fall back to derivative-based detection if changepoint detection failed or not requested
    if not use_changepoint_detection:
        # Smooth signal to reduce noise
        smoothed_signal = savgol_filter(signal, 51, 3)

        # Calculate first derivative to find sharp changes
        derivative = np.diff(smoothed_signal, prepend=smoothed_signal[0])
        ...
        threshold = deriv_median + (5.0 / segmentation_sensitivity) * deriv_mad

        # Find potential changepoints
        potential_changepoints = np.where(deriv_abs > threshold)[0]
        ...
        if len(potential_changepoints) > 0:
            ...
            for i in range(1, len(potential_changepoints)):
                if potential_changepoints[i] - potential_changepoints[i - 1] <= window_size // 2:
                    current_group.append(potential_changepoints[i])
                else:
                    # Use median of group as the changepoint
    ...
    # Ensure we have at least the endpoints for segmentation
    all_points = [0] + changepoints + [len(signal)]
    segments = list(zip(all_points[:-1], all_points[1:]))

    print(f"Signal divided into {len(segments)} segments at points {changepoints}")

    # Step 2: Calculate baseline for each segment
    segment_baselines = []
    for start, end in segments:
        ...
        # Use robust baseline estimation (10th percentile works well for signals with transients)
        baseline = np.percentile(segment, 10)
        segment_baselines.append((start, end, baseline))

    # Step 3: Align segments progressively
    if len(segment_baselines) > 1:
        # Use first segment as reference
        reference_baseline = segment_baselines[0][2]

        # Apply corrections to subsequent segments
        for i, (start, end, baseline) in enumerate(segment_baselines[1:], 1):
            offset = baseline - reference_baseline

            # Check if offset is significant
            signal_range = np.percentile(signal, 95) - np.percentile(signal, 5)
            if abs(offset) > signal_range * 0.01:  # Only correct if >1% of signal range
                print(f"Aligning segment {i} (points {start}-{end}): offsetting by {offset:.4f}")

                if smooth_transitions:
                    # Apply smooth transition at the boundary
                    trans_size = min(window_size, (end - start) // 4)

                    # Full correction for most of segment
                    aligned_signal[start + trans_size:end] -= offset

                    # Gradual transition at beginning of segment
                    weights = np.linspace(0, 1, trans_size)
                    for j in range(trans_size):
                        if start + j < len(aligned_signal):
                            aligned_signal[start + j] -= offset * weights[j]
                else:
...
def connect_jump_selection_events(self):
    ...
    # Update status
    self.baseline_status.config(
        text="Click on the plot to add jump points, right-click to remove.",
        fg="blue")


def on_jump_selection_click(self, event):
    """Handle mouse clicks for adding/removing jump points"""
    if event.inaxes not in [self.ax1, self.ax2]:
        return

    # Determine which signal we're working with
    is_primary = event.inaxes == self.ax1

    time_array = self.processed_time if is_primary else self.secondary_processed_time
    signal = self.processed_signal if is_primary else self.secondary_processed_signal

    # Convert click position (minutes) to array index (seconds)
    ...
    if event.button == 1:  # Left click
        ...
        too_close = False
        for point in self.manual_jump_points:
            if abs(point - idx) < len(time_array) * 0.01:  # Within 1% of signal length
                too_close = True
                break

        if not too_close:
            self.manual_jump_points.append(idx)
            self.manual_jump_points.sort()

            # Update display
            self.highlight_manual_jump_points()
            self.baseline_status.config(
                text=f"Added jump point at {time_array[idx]:.2f}s. Total: {len(self.manual_jump_points)}",
                fg="green")

    elif event.button == 3:  # Right click
        # Find any points close to click
        for i, point in enumerate(self.manual_jump_points):
            if abs(point - idx) < len(time_array) * 0.01:
                del self.manual_jump_points[i]

                # Update display
                self.highlight_manual_jump_points()
                self.baseline_status.config(
                    text=f"Removed jump point at {time_array[idx]:.2f}s. Remaining: {len(self.manual_jump_points)}",
                    fg="blue")
                break
...
def robust_baseline_alignment(signal, time,
    ...
    """
    Enhanced baseline alignment that combines automatic detection with optional manual guidance.
    ...
    time : array
        Corresponding time points
    manual_points : list, optional
        User-specified jump indices to ensure detection of difficult jumps
    detection_sensitivity : float
        Value between 0.1-3.0 controlling detection sensitivity (higher=more sensitive)
    transition_smoothness : float
        Value between 0-1 controlling how smooth transitions should be
    ...
    # 1. DETECTION PHASE - Find potential jump points
    jump_points = []

    # 1a. Calculate metrics for jump detection
    smooth_window = max(51, int(len(signal) * 0.01))  # Dynamic window size
    if smooth_window % 2 == 0:
        smooth_window += 1  # Ensure odd window for savgol

    # Apply smoothing to reduce noise influence
    try:
        from scipy.signal import savgol_filter
        smoothed = savgol_filter(signal, smooth_window, 3)
    except:
        from scipy.ndimage import uniform_filter1d
        smoothed = uniform_filter1d(signal, size=smooth_window)

    # Calculate first and second derivatives
    first_deriv = np.diff(smoothed, prepend=smoothed[0])
    ...
    # Adaptive threshold based on signal properties and sensitivity
    threshold = deriv_median + (5.0 / detection_sensitivity) * deriv_mad

    # 1b. Find points exceeding the threshold (automatic detection)
    auto_jump_candidates = np.where(abs_deriv > threshold)[0]

    # 1c. Cluster adjacent points and get most significant within each cluster
    if len(auto_jump_candidates) > 0:
        clusters = []
        current_cluster = [auto_jump_candidates[0]]

        for i in range(1, len(auto_jump_candidates)):
            if auto_jump_candidates[i] - auto_jump_candidates[i - 1] <= smooth_window // 2:
                current_cluster.append(auto_jump_candidates[i])
            else:
                # Find most significant point in cluster (max absolute derivative)
                best_idx = current_cluster[np.argmax(abs_deriv[current_cluster])]
                clusters.append(best_idx)
                current_cluster = [auto_jump_candidates[i]]
        ...
        jump_points.extend(clusters)

    # 1d. Add manually specified points if provided
    if manual_points is not None:
        for point in manual_points:
            # Only add if not too close to already detected points
            if not any(abs(point - jp) < smooth_window // 4 for jp in jump_points):
                jump_points.append(point)

        # Re-sort all points
        jump_points.sort()

    # 2. VERIFICATION PHASE - Validate jump significance
    verified_jumps = []
    for jump in jump_points:
        # Calculate significance of the jump (change in baseline)
        pre_jump = max(0, jump - smooth_window)
        post_jump = min(len(signal), jump + smooth_window)

        pre_baseline = np.median(signal[pre_jump:jump])
        post_baseline = np.median(signal[jump:post_jump])

        jump_size = abs(post_baseline - pre_baseline)
        signal_std = np.std(signal[max(0, jump - smooth_window * 2):jump])

        # Only keep significant jumps (> 3 sigma or manually specified)
        is_significant = jump_size > 3 * signal_std
        is_manual = manual_points is not None and jump in manual_points

        if is_significant or is_manual:
            verified_jumps.append(jump)

    print(f"Detected {len(verified_jumps)} significant jump points")

    # 3. ALIGNMENT PHASE - Adjust baselines
    if verified_jumps:
        # Add start and end points for complete segmentation
        segments = [0] + verified_jumps + [len(signal)]
        ...
        for i in range(len(segments) - 1):
            ...
            # Get segment with padding to avoid edges
            seg_start = max(0, start + smooth_window // 10)
            seg_end = min(len(signal), end - smooth_window // 10)

            # Skip if segment is too small
            if seg_end - seg_start < smooth_window // 2:
                continue

            # Use robust estimator for baseline (10th percentile)
            baseline = np.percentile(signal[seg_start:seg_end], 10)
            baselines.append((start, end, baseline))

        # Align segments to the first segment's baseline
        if baselines:
            reference_baseline = baselines[0][2]

            for start, end, baseline in baselines[1:]:
                offset = baseline - reference_baseline

                # Skip tiny offsets
                if abs(offset) < 0.01 * (np.max(signal) - np.min(signal)):
                    continue

                # Calculate transition size based on smoothness parameter
                transition_size = int(smooth_window * transition_smoothness)

                # Apply the offset with smooth transition
                if transition_size > 0 and transition_size < (end - start) // 2:
                    # Create smooth transition at the segment start
                    for i in range(transition_size):
                        if start + i < len(aligned_signal):
                            # Use cubic ease-in function for smoother transition
                            t = i / transition_size
                            weight = t * t * (3 - 2 * t)  # Smoother S-curve
                            aligned_signal[start + i] -= offset * weight

                    # Apply full correction after transition
                    aligned_signal[start + transition_size:end] -= offset
                else:
                    # Simple offset without transition for small segments
                    aligned_signal[start:end] -= offset

    return aligned_signal, verified_jumps
...
class PhotometryViewer:
    def __init__(self, root, file_path=None):
        ...
        def setup_keyboard_shortcuts(self):
            ...
            self.root.bind('<Control_L>', lambda event: self._set_ctrl_pressed(True))
            self.root.bind('<KeyRelease-Control_L>', lambda event: self._set_ctrl_pressed(False))
            self.root.bind('<Control_R>', lambda event: self._set_ctrl_pressed(True))
            self.root.bind('<KeyRelease-Control_R>', lambda event: self._set_ctrl_pressed(False))

        def _set_ctrl_pressed(self, pressed):
            """Set the state of the Ctrl key."""
    ...
    def open_baseline_alignment_window(self):
        """Open the baseline alignment options window"""
        align_window = tk.Toplevel(self.root)
        align_window.title("Baseline Alignment")
        align_window.geometry("600x500")
        align_window.transient(self.root)
        ...
        tk.Label(align_frame, text="Alignment Method:", font=('Arial', font_size, 'bold')).pack(anchor="w", pady=(0, 5))

        self.alignment_mode_var = tk.StringVar(value="simple")
        modes_frame = tk.Frame(align_frame)
        modes_frame.pack(fill=tk.X, pady=(0, 15))

        tk.Radiobutton(modes_frame, text="Simple Alignment (Single Jump)",
                       variable=self.alignment_mode_var,
                       value="simple",
                       font=('Arial', font_size)).pack(side=tk.LEFT, padx=10)

        tk.Radiobutton(modes_frame, text="Advanced Alignment (Multiple Jumps)",
                       variable=self.alignment_mode_var,
                       value="advanced",
                       font=('Arial', font_size)).pack(side=tk.LEFT, padx=10)
        ...
        tk.Scale(sensitivity_frame, from_=0.1, to=3.0, resolution=0.1,
                 variable=self.sensitivity_var,
                 orient=tk.HORIZONTAL, length=350).pack(side=tk.LEFT, fill=tk.X, expand=True)

        # Add description
        tk.Label(align_frame,
                 text="Higher sensitivity detects smaller jumps but may find false positives.\nLower sensitivity only finds major jumps.",
                 font=('Arial', font_size - 1),
                 fg="gray",
                 justify=tk.LEFT).pack(anchor="w", pady=(0, 15))

        # Smoothness slider for transitions
        smoothness_frame = tk.Frame(align_frame)
        smoothness_frame.pack(fill=tk.X, pady=10)

        tk.Label(smoothness_frame, text="Transition Smoothness:",
                 font=('Arial', font_size)).pack(side=tk.LEFT, padx=(0, 10))

        self.smoothness_var = tk.DoubleVar(value=0.5)
        tk.Scale(smoothness_frame, from_=0.0, to=1.0, resolution=0.1,
                 variable=self.smoothness_var,
                 orient=tk.HORIZONTAL, length=350).pack(side=tk.LEFT, fill=tk.X, expand=True)

        # Signal selection
        signal_frame = tk.LabelFrame(align_frame, text="Signal to Align", font=('Arial', font_size, 'bold'))
        signal_frame.pack(fill=tk.X, pady=15)

        self.align_signal_var = tk.StringVar(value="Primary")

        # Create radio buttons for signal selection with a secondary file check
        has_secondary = hasattr(self, 'secondary_processed_signal') and self.secondary_processed_signal is not None

        tk.Radiobutton(signal_frame, text="Primary Signal",
                       variable=self.align_signal_var,
                       value="Primary",
                       font=('Arial', font_size)).pack(side=tk.LEFT, padx=20, pady=5)

        tk.Radiobutton(signal_frame, text="Secondary Signal",
                       variable=self.align_signal_var,
                       value="Secondary",
                       state=tk.NORMAL if has_secondary else tk.DISABLED,
                       font=('Arial', font_size)).pack(side=tk.LEFT, padx=20, pady=5)

        tk.Radiobutton(signal_frame, text="Both",
                       variable=self.align_signal_var,
                       value="Both",
                       state=tk.NORMAL if has_secondary else tk.DISABLED,
                       font=('Arial', font_size)).pack(side=tk.LEFT, padx=20, pady=5)
        ...
        # Manual guidance frame
        manual_frame = tk.LabelFrame(align_frame, text="Manual Jump Point Guidance", font=('Arial', font_size, 'bold'))
        manual_frame.pack(fill=tk.X, pady=15)

        self.manual_guidance_var = tk.BooleanVar(value=False)
        tk.Checkbutton(manual_frame,
                       text="Enable manual selection of jump points (select after clicking Detect)",
                       variable=self.manual_guidance_var,
                       font=('Arial', font_size)).pack(anchor="w", padx=10, pady=5)

        manual_instruction = tk.Label(manual_frame,
                                      text="After detection, click on the plot to add jump points.\nRight-click to remove points.",
                                      font=('Arial', font_size - 1),
                                      fg="gray", justify=tk.LEFT)
        manual_instruction.pack(anchor="w", padx=10, pady=5)

        # Add extreme jump handling options
        extreme_frame = tk.LabelFrame(align_frame, text="Advanced Options", font=('Arial', font_size, 'bold'))
        extreme_frame.pack(fill=tk.X, pady=10)

        self.handle_extreme_var = tk.BooleanVar(value=True)
        tk.Checkbutton(extreme_frame,
                       text="Force correction of extremely large jumps (>10% signal range)",
                       variable=self.handle_extreme_var,
                       font=('Arial', font_size)).pack(anchor="w", padx=10, pady=5)

        self.iterative_var = tk.BooleanVar(value=True)
        tk.Checkbutton(extreme_frame,
                       text="Use iterative alignment for better results",
                       variable=self.iterative_var,
                       font=('Arial', font_size)).pack(anchor="w", padx=10, pady=5)

        # Max iterations slider (only visible if iterative is checked)
        iter_frame = tk.Frame(extreme_frame)
        iter_frame.pack(fill=tk.X, padx=20, pady=5)

        tk.Label(iter_frame, text="Max Iterations:",
                 font=('Arial', font_size - 1)).pack(side=tk.LEFT, padx=(0, 5))

        self.max_iterations_var = tk.IntVar(value=3)
        tk.Scale(iter_frame, from_=1, to=5, resolution=1, orient=tk.HORIZONTAL,
                 variable=self.max_iterations_var, length=150).pack(side=tk.LEFT)

        # Buttons frame - EXISTING CODE CONTINUES
        ...
        self.detect_jumps_btn = tk.Button(
            ...
            font=('Arial', font_size),
            bg="lightblue",
            command=self.detect_baseline_jumps)
        self.detect_jumps_btn.pack(side=tk.LEFT, padx=10)

        # Apply button
        tk.Button(btn_frame,
                  text="Apply Alignment",
                  command=self.run_baseline_alignment,
                  bg="lightgreen",
                  font=('Arial', font_size)).pack(side=tk.LEFT, padx=10)
        ...
        self.clear_markers_btn = tk.Button(
            ...
            font=('Arial', font_size),
            bg="pink",
            command=self.clear_jump_markers)
        self.clear_markers_btn.pack(side=tk.LEFT, padx=10)

        # Status label
        self.baseline_status = tk.Label(
            align_frame,
            text="Ready for alignment. Click 'Detect Jumps' to preview.",
            font=('Arial', font_size - 1),
            fg="blue")
        self.baseline_status.pack(fill=tk.X, pady=10)

        # Store reference to window and initialize state variables
        self.align_window = align_window
        self.manual_jump_points = []
        self.auto_jump_points = []
        self.jump_click_cid = None

        # Initialize the combined_jumps flag for hybrid mode
        self.combined_jumps = []

    def detect_baseline_jumps(self):
        """Detect and display potential baseline jump points in the signal"""
        # Reset previous jump points
        self.auto_jump_points = []

        # Determine which signal to process
        ...
        if signal_to_align in ["Primary", "Both"] and hasattr(self,
            ...
            # Get jump points using the selected method
            if self.alignment_mode_var.get() == "simple":
                # Use simpler method for single jump
                jump_points = self.detect_single_jump(self.processed_signal, self.processed_time, sensitivity)
            else:
                # Use advanced method for multiple jumps
                jump_points = self.detect_multiple_jumps(self.processed_signal, self.processed_time, sensitivity)

            if jump_points:
                self.auto_jump_points.extend([(idx, "primary") for idx in jump_points])
                self.highlight_jump_points(jump_points, "primary")
        ...
        if signal_to_align in ["Secondary", "Both"] and hasattr(self,
                                                                'secondary_processed_signal') and self.secondary_processed_signal is not None:
            if self.alignment_mode_var.get() == "simple":
                jump_points = self.detect_single_jump(self.secondary_processed_signal, self.secondary_processed_time,
                                                      sensitivity)
            else:
                jump_points = self.detect_multiple_jumps(self.secondary_processed_signal, self.secondary_processed_time,
                                                         sensitivity)

            if jump_points:
                self.auto_jump_points.extend([(idx, "secondary") for idx in jump_points])
                self.highlight_jump_points(jump_points, "secondary")

        # Update status with count of detected jumps
        total_jumps = len(self.auto_jump_points)
        self.baseline_status.config(
            text=f"Detected {total_jumps} possible jump points. " +
                 ("Click on plot to add/remove points." if self.manual_guidance_var.get() else ""),
            fg="green" if total_jumps > 0 else "orange")

        # Enable manual selection if requested
    ...
    def highlight_jump_points(self, jump_points, signal_type="primary"):
        """Highlight detected jump points on the plot"""
        if not jump_points:
            return
    ...
    def clear_jump_markers(self):
        """Clear all jump point markers"""
        # Clear auto markers
        for ax in [self.ax1, self.ax2]:
            for artist in ax.get_lines():
                if hasattr(artist, 'is_jump_marker') or hasattr(artist, 'is_manual_jump_marker'):
                    artist.remove()
    ...
    def run_baseline_alignment(self):
        """Apply baseline alignment with current settings and iterative correction"""
        # Get parameters
        sensitivity = self.sensitivity_var.get()
        smoothness = self.smoothness_var.get()
        mode = self.alignment_mode_var.get()
        ...
        # Get parameters for the new options
        use_iterative = self.iterative_var.get()
        handle_extreme = self.handle_extreme_var.get()
        max_iterations = self.max_iterations_var.get()

        # Update status
        self.baseline_status.config(text="Running alignment...", fg="blue")
        self.root.update_idletasks()  # Update UI immediately to show status

        # Collect jump points from manual or auto detection
        all_primary_jumps = []
        all_secondary_jumps = []

        # PRIORITIZE MANUAL MARKERS - This change ensures manual points are used
        if self.manual_guidance_var.get() and self.manual_jump_points:
            # Use manual points if available - they take precedence
            print(f"Using {len(self.manual_jump_points)} manual jump points")
            for idx, signal_type in self.manual_jump_points:
                if signal_type == "primary":
                    all_primary_jumps.append(idx)
                else:
                    all_secondary_jumps.append(idx)
        else:
            # Use auto-detected points if no manual points
            print(f"Using {len(self.auto_jump_points)} auto-detected jump points")
            for idx, signal_type in self.auto_jump_points:
                if signal_type == "primary":
                    all_primary_jumps.append(idx)
                else:
        ...
        # Sort points
        all_primary_jumps.sort()
        all_secondary_jumps.sort()

        print(f"Primary jumps: {all_primary_jumps}")
        print(f"Secondary jumps: {all_secondary_jumps}")

        # Track if any signals were updated
        signals_updated = []

        # Apply alignment to primary signal if selected
        if signal_selection in ["Primary", "Both"] and self.processed_signal is not None:
            # For simple mode with no detected jumps, try to find one
            if mode == "simple" and not all_primary_jumps:
                all_primary_jumps = self.detect_single_jump(self.processed_signal, self.processed_time, sensitivity)
                print(f"Auto-detected primary jump at {all_primary_jumps[0] if all_primary_jumps else 'none'}")

            # Decide which alignment approach to use
            if use_iterative:
                # Use iterative alignment
                print(f"Using iterative alignment for primary signal with max {max_iterations} iterations")
                aligned_signal = self.iterative_align_signal(
                    self.processed_signal,
                    self.processed_time,
                    all_primary_jumps,
                    mode,
                ...
                if mode == "simple" and len(all_primary_jumps) > 0:
                    print(f"Using simple alignment for primary signal at jump {all_primary_jumps[0]}")
                    aligned_signal = self.align_simple_jump(
                        self.processed_signal,
                        self.processed_time,
                        all_primary_jumps[0] if all_primary_jumps else None,
                        smoothness
                    )
                else:
                    print(f"Using multiple jump alignment for primary signal with {len(all_primary_jumps)} jumps")
                    aligned_signal = self.align_multiple_jumps(
                        self.processed_signal,
                        self.processed_time,
                        all_primary_jumps,
                        smoothness
            ...
            # Force alignment of extreme jumps if enabled
            if handle_extreme and aligned_signal is not None:
                extreme_jumps = self.check_for_extreme_jumps(aligned_signal, self.processed_time)
                if extreme_jumps:
                    print(f"Handling {len(extreme_jumps)} extreme jumps in primary signal")
                    aligned_signal = self.force_align_extreme_jumps(aligned_signal, self.processed_time, extreme_jumps)

            # Update the signal
            if aligned_signal is not None:
                self.processed_signal = aligned_signal
                self.line.set_ydata(aligned_signal)
                signals_updated.append("primary")
                print("Updated primary signal with aligned data")

        # Apply alignment to secondary signal if selected
        if signal_selection in ["Secondary", "Both"] and hasattr(self,
                                                                 'secondary_processed_signal') and self.secondary_processed_signal is not None:
            # For simple mode with no detected jumps, try to find one
            if mode == "simple" and not all_secondary_jumps:
                all_secondary_jumps = self.detect_single_jump(
                    self.secondary_processed_signal,
                    self.secondary_processed_time,
                    sensitivity
                )
                print(f"Auto-detected secondary jump at {all_secondary_jumps[0] if all_secondary_jumps else 'none'}")

            # Decide which alignment approach to use
            if use_iterative:
                # Use iterative alignment
                print(f"Using iterative alignment for secondary signal with max {max_iterations} iterations")
                aligned_signal = self.iterative_align_signal(
                    self.secondary_processed_signal,
                    self.secondary_processed_time,
                    all_secondary_jumps,
                ...
                # Use standard alignment
                if mode == "simple" and len(all_secondary_jumps) > 0:
                    print(f"Using simple alignment for secondary signal at jump {all_secondary_jumps[0]}")
                    aligned_signal = self.align_simple_jump(
                        self.secondary_processed_signal,
                        self.secondary_processed_time,
                        all_secondary_jumps[0] if all_secondary_jumps else None,
                        smoothness
                    )
                else:
                    print(f"Using multiple jump alignment for secondary signal with {len(all_secondary_jumps)} jumps")
                    aligned_signal = self.align_multiple_jumps(
                        self.secondary_processed_signal,
                        self.secondary_processed_time,
                        all_secondary_jumps,
            ...
            # Force alignment of extreme jumps if enabled
            if handle_extreme and aligned_signal is not None:
                extreme_jumps = self.check_for_extreme_jumps(aligned_signal, self.secondary_processed_time)
                if extreme_jumps:
                    print(f"Handling {len(extreme_jumps)} extreme jumps in secondary signal")
                    aligned_signal = self.force_align_extreme_jumps(aligned_signal, self.secondary_processed_time,
                                                                    extreme_jumps)

            # Update the signal
            if aligned_signal is not None:
                self.secondary_processed_signal = aligned_signal
                self.secondary_line.set_ydata(aligned_signal)
                signals_updated.append("secondary")
                print("Updated secondary signal with aligned data")

        # Clear markers and reset interface
        self.clear_jump_markers()
        if self.jump_click_cid is not None:
            self.canvas.mpl_disconnect(self.jump_click_cid)
            self.jump_click_cid = None
        ...
        # Update status with clear message about what was done
        if not signals_updated:
            status_text = "No signals were updated - alignment may have failed or no jumps detected"
            status_color = "orange"
        else:
            ...
            if handle_extreme:
                features.append("extreme jump handling")

            method_text = "simple" if mode == "simple" else "advanced"
            features_text = " with " + " and ".join(features) if features else ""
            signals_text = " and ".join(signals_updated)

            status_text = f"Applied {method_text} alignment{features_text} to {signals_text} signal(s)"
            status_color = "green"
    ...
    def force_align_extreme_jumps(self, signal, time, extreme_jumps):
        ...
        aligned_signal = signal.copy()
        window_size = min(int(len(signal) * 0.05), 500)  # 5% of signal or 500 points
        ...
        # Calculate robust baseline for each segment
        segment_data = []
        for i in range(len(segments) - 1):
            start_idx = segments[i]
            end_idx = segments[i + 1]

            # Ensure segment is large enough
            if end_idx - start_idx < window_size // 2:
                continue

            # Calculate baseline using 10th percentile
            segment = aligned_signal[start_idx:end_idx]
            baseline = np.percentile(segment, 10)
        ...
        # Force alignment to first segment baseline
        if len(segment_data) > 1:
            reference_baseline = segment_data[0]['baseline']

            for segment in segment_data[1:]:
                # Calculate offset
                offset = segment['baseline'] - reference_baseline

                # Skip very small offsets (< 1% of signal range)
                if abs(offset) < 0.01 * (np.max(signal) - np.min(signal)):
                    continue

                print(f"Forcing alignment of extreme jump at {segment['start']} with offset {offset:.2f}")

                # Apply smooth transition at boundary
                transition_size = min(window_size, (segment['end'] - segment['start']) // 4)
    ...
    def run_baseline_alignment(self):
        """Run the baseline alignment based on user settings"""
        # Get settings
        mode = self.alignment_mode_var.get()
        sensitivity = self.sensitivity_var.get()
        target = self.align_signal_var.get()

        # Process primary signal if selected
        if target in ["Primary", "Both"]:
            if mode == "simple":
                corrected_signal, shift_points = auto_align_baselines(
                    self.processed_signal,
                    self.processed_time,
                    threshold_pct=2.0 / sensitivity  # Adjust threshold based on sensitivity
                )
            else:
                corrected_signal, shift_points = advanced_baseline_alignment(
                    self.processed_signal,
                    self.processed_time,
                    segmentation_sensitivity=sensitivity
                )

            # Update the primary signal
            self.processed_signal = corrected_signal
            self.line.set_ydata(corrected_signal)

        # Process secondary signal if selected
        if target in ["Secondary", "Both"] and hasattr(self, 'secondary_processed_signal'):
            if mode == "simple":
                corrected_signal, shift_points = auto_align_baselines(
                    self.secondary_processed_signal,
                    self.secondary_processed_time,
                    threshold_pct=2.0 / sensitivity
                )
            else:
                corrected_signal, shift_points = advanced_baseline_alignment(
                    self.secondary_processed_signal,
            ...
            # Update the secondary signal
            self.secondary_processed_signal = corrected_signal
            self.secondary_line.set_ydata(corrected_signal)

        # Redraw
        self.canvas.draw_idle()

        # Update status
        self.status_label.config(text=f"Baseline alignment completed with {mode} mode", fg="green")
    ...
    def zoom_x(self, in_out="in"):
        def _handle_zoom(self, direction, xdata, ydata, ax):
            """
            Handle zooming for the plot. Only zoom the x-axis when Ctrl is pressed.
            """
            # Determine zoom factor
            zoom_factor = 1.2 if direction == 'up' else 0.8
            ...
            if self.ctrl_pressed:
                # Zoom only the x-axis
                x_range = xlim[1] - xlim[0]
                x_center = xdata
                new_xmin = x_center - (x_range * zoom_factor) / 2
                new_xmax = x_center + (x_range * zoom_factor) / 2
                ax.set_xlim(new_xmin, new_xmax)
            else:
                # Default behavior: zoom both x and y axes
                x_range = xlim[1] - xlim[0]
                y_range = ylim[1] - ylim[0]
                x_center, y_center = xdata, ydata
        
                new_xmin = x_center - (x_range * zoom_factor) / 2
                new_xmax = x_center + (x_range * zoom_factor) / 2
                new_ymin = y_center - (y_range * zoom_factor) / 2
                new_ymax = y_center + (y_range * zoom_factor) / 2
